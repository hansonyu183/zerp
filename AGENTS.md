...
- 用户操作使用 `POST application/json` 和 `/{domain}/{entity}/{action}`，辅助读取使用所属实体 GET，不设独立动作权限或 CSRF，复用有效 Session 与已有账簿范围；方法规范和逐项例外见 `docs/adr/0061-action-post-auxiliary-get.md`，响应包络为 `{code, errorKey, message, data, requestId}`；失败响应使用稳定 `errorKey` 表达业务语义，前端只按 `errorKey` 分支或映射用户提示，`message` 仅用于诊断和默认说明。
