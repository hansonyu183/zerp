...
- `validation` 是自动化门禁聚合，`full-validation` 是最终可合并证据。GitHub 的 `contracts`、`frontend`、`backend`、`containers`、`e2e` 和 `validation` 必须按影响矩阵成功；无需预览的 PR 由工作流为最新 SHA 发布 `full-validation`。需要预览的 PR 先只发布 `preview-required`，必须使用 `make preview-deploy PREVIEW_PR=<number> PREVIEW_REF=<PR-head-full-sha>` 部署准确提交，再由具备 write 权限的非 Bot 用户完成固定预览人工验收，才允许为同一 SHA 发布 `full-validation`。新提交使之前的预览和验收证据失效。
