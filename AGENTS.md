...
- 创建 PR 前必须获取最新 `origin/main`，将功能提交重放到该提交，解决冲突并保持工作区干净；检查相对 `origin/main` 的完整差异，再依次运行 `make pre-push-plan` 和聚焦的 `make pre-push`。禁止先创建 PR，再因分支落后而 rebase 或强推。通过后推送并创建目标为 `main` 的 Draft PR；最后一次确认分支基于最新 `main` 后转为 Ready。Draft 不得用人工失败伪装未就绪；只有 Ready 的最新提交才运行并满足完整自动验证。
...
- 合并前必须确认 PR 最新 SHA 的全部必需检查成功；适用固定预览时，自动门禁就绪后再部署并完成人工验收。
