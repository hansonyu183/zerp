...
- 形成可验收提交后运行 `make pre-push`；门禁按 `scripts/change-impact.sh` 分为文档、验证工具和应用影响三级。文档与验证工具只运行对应的格式、文档、Shell、工作流和门禁行为检查；应用影响变更必须通过生成检查、全栈质量门禁和隔离 E2E。
- 应用影响变更通过本地门禁后，使用 `make preview-deploy PREVIEW_REF=<commit>` 发布准确提交；不得用包含未提交修改的工作区代替。文档和验证工具变更无需部署应用预览。
- 推送后创建草稿 PR；GitHub 必需检查全部通过，且应用影响变更完成固定预览人工验收后，才允许人工合并。
