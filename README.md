...
```text
POST /auth/user/signin   # 登录并由后端写入会话 Cookie
POST /auth/user/session  # 恢复用户、菜单、权限及 CSRF Token
POST /auth/user/signout  # 注销并由后端清理会话 Cookie
```
