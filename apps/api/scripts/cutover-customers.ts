  ...
  try {
    if (!values.apply) {
      ...
      if (
        ...
        throw new Error(
          'apply requires --baseline, --backup --actor-id and --writers-frozen',
        )
      const backupFile = z
        .object({
          path: z.string().min(1),
      ...
      if (process.env.ZERP_RELEASE_SHA !== manifest.targetReleaseSha)
        throw new Error('customer_cutover_release_mismatch')
      for (const file of [manifest.database, manifest.attachments]) {
        ...
        if (hash.digest('hex') !== file.sha256)
          throw new Error('customer_cutover_backup_digest_mismatch')
      }
      const baseline = z
  ...
  process.stderr.write(
    JSON.stringify(
      error instanceof CustomerCutoverError
        ? { error: error.reason, review: error.review }
        : { error: 'customer_cutover_failed' },
    ) + '\n',
  )
  process.exitCode = 1
}
