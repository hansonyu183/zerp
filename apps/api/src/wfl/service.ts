...
export interface WflDefinitionView {
  ...
  availableRuntimeActions: WflDefinitionRuntimeAction[]
  canDelete: boolean
}

export const wflDefinitionRuntimeActions = ['enable', 'disable'] as const
export type WflDefinitionRuntimeAction =
  (typeof wflDefinitionRuntimeActions)[number]

export function availableWflDefinitionRuntimeActions(
  definition: {
    status: ApprovalStatus
    enabled: boolean
...
export function canDeleteWflDefinition(
  definition: { status: ApprovalStatus; submittedBy: string },
  actor: ApprovalActor,
): boolean {
  return (
...
export class WflService implements PlanExecutor<WflApplicationPlan> {
  ...
  async query(
    ...
    const views = await Promise.all(
      [...subjects.entries()].map(async ([subjectId, subject]) => ({
        subjectId,
        code: subject.code,
        latestApproved: subject.latestApprovedId
          ? await this.readDefinition(this.db, subject.latestApprovedId, actor)
          : null,
        openCandidate: subject.openCandidateId
          ? await this.readDefinition(this.db, subject.openCandidateId, actor)
          : null,
      })),
    )
    ...
    const filtered = views.filter(
      (item) =>
        (!input.code || item.code === input.code) &&
        (!keyword ||
          item.code.toLowerCase().includes(keyword) ||
          item.latestApproved?.compiledGraph.name
            .toLowerCase()
            .includes(keyword) ||
          item.openCandidate?.compiledGraph.name
            .toLowerCase()
            .includes(keyword)),
    )
    ...
    return { items: selected, total: filtered.length, page, pageSize }
  }
  ...
  async versions(
    ...
    const rows = await this.db
      ...
      .execute()
    return Promise.all(
      rows.map((row) => this.readDefinition(this.db, row.id, actor)),
    )
  }
  ...
  async auditHistory(
    ...
    return rows.map((row) => ({
      ...
      action: row.action as WflDefinitionAuditView['action'],
  ...
  async delete(
    ...
    return this.db.transaction().execute(async (tx) => {
      ...
      if (String(entry.revision) !== input.expectedRevision)
      ...
      if (actor.trusted !== true && entry.submitted_by !== actor.id)
        throw new WflApplicationError('approval_invalid_actor')
  ...
  async queryInstances(
    ...
    const filtered = rows.filter(
      (row) =>
        ...
        (!keyword ||
          row.definition_code.toLowerCase().includes(keyword) ||
          row.definition_name.toLowerCase().includes(keyword)),
    )
    const page = input.page ?? 1
    const pageSize = input.pageSize ?? 20
    const ids = filtered
    ...
    return {
      items: await Promise.all(
        ids.map((id) => this.readInstance(this.db, id, actor)),
      ),
      ...
      items: await Promise.all(
        selected.map(async (row) => {
          ...
          return {
            ...
            name: row.name,
            enabled: row.enabled,
            compiledGraph: view.compiledGraph,
          }
        }),
      ),
      total: filtered.length,
    ...
    return this.db.transaction().execute(async (tx) => {
      ...
      if (
        ...
        const prior = await tx
          .selectFrom('wfl_action_results')
          .select(['fingerprint', 'active'])
          .where('instance_id', '=', input.processId)
          .where('source_node_id', '=', node.nodeId)
          .where(
            'script_position',
            'like',
            `edge:${node.nodeKey}:${input.targetNodeKey}:%`,
          )
          .forUpdate()
          .executeTakeFirst()
      ...
      if (!node.availableActions.includes(input.action))
        throw new WflApplicationError('wfl_action_unavailable')
      if (input.action === 'OPEN_DOCUMENT') {
        ...
        const prior = await tx
          ...
          .where('instance_id', '=', input.processId)
          .where('source_node_id', '=', node.nodeId)
          .where('script_position', '=', position)
          .forUpdate()
          .executeTakeFirst()
        if (!prior) {
          await tx
            .insertInto('wfl_action_results')
            .values({
              id: ulid(),
              instance_id: input.processId,
          ...
          throw new WflApplicationError('wfl_action_conflict')
      ...
      const prior = await tx
        ...
        .forUpdate()
        .executeTakeFirst()
      if (prior?.active && prior.fingerprint === fingerprint)
        return this.readInstance(tx, input.processId, actor)
      const portInput = {
        entity: node.entity,
      ...
      if (input.action === 'APPROVE_CHILD')
        await this.vouPort.approveChild(tx, portInput)
      else if (input.action === 'REJECT_CHILD') {
        if (!input.reason?.trim())
          throw new WflApplicationError('approval_reason_required')
        await this.vouPort.rejectChild(tx, {
          ...portInput,
          reason: input.reason.trim(),
        })
      } else if (input.action === 'RETRY_CHILD')
        await this.vouPort.retryChild(tx, portInput)
      else {
        await tx
          .updateTable('wfl_action_results')
          .set({ active: false, target_document_id: null })
          .where('target_document_id', '=', node.documentId)
          .execute()
        await tx
          .updateTable('wfl_instance_nodes')
      ...
      if (prior)
        await tx
          ...
          .set({
            fingerprint,
            target_document_id:
              input.action === 'CANCEL_CHILD' ? null : node.documentId,
            active: input.action !== 'CANCEL_CHILD',
            created_at: now,
          })
          .where('instance_id', '=', input.processId)
          .where('source_node_id', '=', node.nodeId)
  ...
  async apply(tx: Transaction<DB>, plan: WflApplicationPlan): Promise<void> {
    ...
    if (plan.action === 'unapprove') {
      ...
      const child = await tx
        ...
        .executeTakeFirst()
      if (child)
        throw new WflApplicationError('wfl_downstream_blocker', [
          { kind: 'WFL_DOCUMENT', id: child.document_id },
        ])
      await tx
        .updateTable('wfl_action_results')
        .set({ active: false })
        ...
        .execute()
      await tx
        .insertInto('wfl_runtime_audits')
        .values({
          id: ulid(),
          instance_id: instance.id,
          action: 'ROOT_UNAPPROVED',
          actor_id: plan.actorId,
          details: json({
            documentId: plan.documentId,
            approvalEntryId: plan.approvalEntryId,
          }),
    ...
    const instanceId = ulid(),
      rootNodeId = ulid()
    await tx
      .insertInto('wfl_instances')
      .values({
        id: instanceId,
        definition_subject_id: match.subject_id,
        approval_entry_id: match.approval_entry_id,
        definition_code: graph.code,
        definition_name: graph.name,
        root_document_id: plan.documentId,
        created_at: new Date(plan.occurredAt),
      })
      .execute()
    await tx
      .insertInto('wfl_instance_nodes')
      .values({
        id: rootNodeId,
        instance_id: instanceId,
        node_key: graph.rootKey,
  ...
  async submit(
    ...
    return this.db.transaction().execute(async (tx) => {
      ...
      if (duplicateCode.rows.length > 0)
        throw new WflApplicationError('wfl_definition_code_conflict')
      const now = new Date()
      if (!subject) {
      ...
      await tx
        ...
        .execute()
      await tx
        .insertInto('wfl_trials')
        .values({
          approval_entry_id: input.submissionId,
          document_id: input.trialDocument.documentId,
          payload_digest: prepared.payloadDigest,
          result: json(prepared.result),
          created_at: now,
          created_by: actor.id,
        })
        .execute()
    ...
    return this.db.transaction().execute(async (tx) => {
      ...
      if (action === 'approve') {
        ...
        const document = await tx
          ...
          .select('entity')
          .where('id', '=', trial.document_id)
          .executeTakeFirst()
        if (!document)
          throw new WflApplicationError('wfl_trial_document_not_found')
        await this.compileAndTrial(
          tx,
          definition.script,
          {
            entity: document.entity as VouEntity,
            documentId: trial.document_id,
          },
          actor,
        )
      }
      if (action === 'unapprove') {
        ...
        if (blocker)
          throw new WflApplicationError('wfl_definition_in_use', [
            { kind: 'WFL_INSTANCE', id: blocker.id },
          ])
      }
      const now = new Date()
  ...
  async setEnabled(
    ...
    return this.db.transaction().execute(async (tx) => {
      ...
      const latest = await tx
        ...
        .orderBy('version_no', 'desc')
        .executeTakeFirst()
      if (latest?.id !== input.approvalEntryId)
        throw new WflApplicationError('approval_not_latest_approved')
      const runtime = await tx
        .selectFrom('wfl_definition_runtime_states')
        .selectAll()
        .where('subject_id', '=', input.subjectId)
        .forUpdate()
        .executeTakeFirst()
      ...
      if (!runtime) {
        if (input.expectedRuntimeRevision !== null)
          throw new WflApplicationError('approval_stale_revision')
        await tx
          .insertInto('wfl_definition_runtime_states')
          .values({
            subject_id: input.subjectId,
            enabled,
            updated_at: now,
      ...
      await tx
        ...
        .set({ enabled, revision, updated_at: now, updated_by: actor.id })
        .where('subject_id', '=', input.subjectId)
        .where('revision', '=', runtime.revision)
        .executeTakeFirstOrThrow()
      return {
        subjectId: input.subjectId,
        approvalEntryId: entry.id,
  ...
  async current(code: string, actor: ApprovalActor) {
    ...
    return {
      subjectId: view.subjectId,
      approvalEntryId: view.submissionId,
      code: view.compiledGraph.code,
      name: view.compiledGraph.name,
  ...
  async trial(
    ...
    const definition = await this.db
      .selectFrom('wfl_definition_versions')
      .select('script')
      .where('approval_entry_id', '=', input.approvalEntryId)
      .executeTakeFirst()
    if (!definition) throw new WflApplicationError('wfl_definition_not_found')
    return this.compileAndTrial(
      this.db,
      definition.script,
      input.document,
      actor,
  ...
  private async createChild(
    ...
    if (
      ...
      input.requestKey.length > 64
    )
      throw new WflApplicationError('wfl_request_key_invalid')
    const target = view.availableTargets.find(
      (item) =>
        item.parentNodeId === node.nodeId &&
        item.targetNodeKey === input.targetNodeKey,
    )
    ...
    if (existing) {
      ...
      `.execute(tx)
      if (consumed.rows[0])
        throw new WflApplicationError('wfl_request_key_consumed')
    }
    ...
    if (!child.documentId || !child.submissionId)
      throw new WflApplicationError('wfl_vou_port_invalid_result')
    const now = new Date()
    if (existing)
      await tx
        .updateTable('wfl_action_results')
        .set({
          fingerprint,
          target_document_id: child.documentId,
          active: true,
          created_at: now,
        })
    ...
    await tx
      ...
      .values({
        id: ulid(),
        instance_id: input.processId,
        node_key: target.targetNodeKey,
        document_id: child.documentId,
  ...
  private async readInstance(
    ...
    const instance = await executor
      ...
      .select([
        'i.id',
        'i.definition_subject_id',
        'i.approval_entry_id',
        'i.definition_code',
        'i.definition_name',
        'i.root_document_id',
        'i.created_at',
        'root.document_no as root_document_no',
        'root.entity as root_entity',
        'v.script',
        'v.compiled_graph',
      ])
    ...
    const rawNodes = await executor
      ...
      .leftJoin('approval_entries as e', (join) =>
      ...
      .where('n.instance_id', '=', processId)
      .orderBy('n.created_at', 'asc')
      .orderBy('n.id', 'asc')
      .execute()
    const activeRows = await executor
      .selectFrom('wfl_action_results')
      .select(['source_node_id', 'script_position'])
      .where('instance_id', '=', processId)
      .where('active', '=', true)
      .execute()
    ...
    for (const row of rawNodes) {
      ...
      const persisted = await readVouPersistence(executor, {
        approvalEntryId: row.submission_id!,
      })
      const evaluated = await this.runtime.run({
        source: instance.script,
        operation: 'evaluate',
        sourceNodeKey: row.node_key,
        input: persisted.payload,
      })
      if (!evaluated.ok || !evaluated.evaluation)
        throw new WflApplicationError('wfl_runtime_failed')
      const targets = evaluated.evaluation.branches.flatMap((branch) => {
        if (!branch.matched) return []
        const edge = graph.edges.find(
          (item) =>
            item.sourceKey === row.node_key &&
            item.targetKey === branch.targetKey,
        )
        const targetNode = graph.nodes.find(
          (item) => item.key === branch.targetKey,
        )
        if (
          !edge ||
          !targetNode ||
        ...
        return [
          {
            ...
            targetEntity: targetNode.entity as VouEntity,
            relation: edge.relation,
            actionName: edge.actionName,
            initial: branch.initial ?? {},
          } satisfies WflAvailableChildTarget,
        ]
      })
    ...
    const nodes: WflInstanceNodeView[] = rawNodes.map((row) => {
      ...
      return {
        ...
        nodeName: graphNode?.name ?? row.node_key,
        documentId: row.document_id,
        documentNo: row.document_no,
        entity,
        submissionId: row.submission_id,
        status: row.status as ApprovalStatus | null,
    ...
    return {
      ...
      definitionSubjectId: instance.definition_subject_id,
      approvalEntryId: instance.approval_entry_id,
      definitionCode: instance.definition_code,
      definitionName: instance.definition_name,
      rootDocumentId: instance.root_document_id,
      rootDocumentNo: instance.root_document_no,
      rootEntity: instance.root_entity as VouEntity,
  ...
  private async compileAndTrial(
    executor: Executor,
    script: string,
    document: { entity: VouEntity; documentId: string },
    actor: ApprovalActor,
  ) {
    if (script.length > 65_536)
      throw new WflApplicationError('wfl_script_too_large')
    const compiled = await this.runtime.run({
      source: script,
      operation: 'compile',
    })
    if (!compiled.ok || !compiled.graph)
      throw new WflApplicationError('wfl_compile_failed')
    if (compiled.graph.nodes.length > 128 || compiled.graph.edges.length > 127)
      throw new WflApplicationError('wfl_resource_limit')
    const root = compiled.graph.nodes.find(
      (node) => node.key === compiled.graph?.rootKey,
    )
    if (!root || root.entity !== document.entity)
      throw new WflApplicationError('wfl_trial_entity_mismatch')
    let persisted
    try {
      persisted = await readVouPersistence(executor, {
    ...
    if (persisted.entity !== document.entity)
      throw new WflApplicationError('wfl_trial_document_not_found')
    const result = await this.runtime.run({
      source: script,
      operation: 'evaluate',
      sourceNodeKey: compiled.graph.rootKey,
      input: persisted.payload,
    })
    if (!result.ok) throw new WflApplicationError('wfl_trial_failed')
    const payloadDigest = createHash('sha256')
      .update(JSON.stringify(persisted.payload))
      .digest('hex')
    return {
      graph: compiled.graph,
      result: result as WflStarlarkResult,
      payloadDigest,
      actorId: actor.id,
    }
  }
  ...
  private async readDefinition(
    ...
    return {
      ...
      revision: String(row.revision),
      script: row.script,
      compiledGraph: row.compiled_graph as unknown as WflStarlarkGraph,
      enabled,
      runtimeRevision:
        row.runtime_revision === null ? null : String(row.runtime_revision),
      ...
      availableRuntimeActions: availableWflDefinitionRuntimeActions(
        {
          status: row.status as ApprovalStatus,
          enabled,
          latestApproved: latestApproved?.id === row.id,
        },
        actor,
      ),
      canDelete: canDeleteWflDefinition(
        {
          status: row.status as ApprovalStatus,
