...
test('ACC opening freezes AUX bill and employee-dimension adoptions until its submission is deleted', async (context) => {
  ...
  await aux.save(
    'employee',
    {
      ...
      employeeCategoryId: employeeV1.employeeCategory?.id ?? null,
      departmentId: employeeV1.department?.id ?? null,
      positionId: employeeV1.position?.id ?? null,
      employmentDate: employeeV1.employmentDate,
      workPhone: employeeV1.workPhone,
      workEmail: employeeV1.workEmail,
      operatingEntityId: employeeV1.operatingEntity?.id ?? null,
...
test('ACC restores f856118f subject templates and independent book access scopes', async (context) => {
  ...
  context.after(async () => {
    try {
      ...
      await db
        .deleteFrom('app_users')
        .where('id', 'in', [creatorId, queryUserId, operateUserId])
        .execute()
    } finally {
      await db.destroy()
    }
  ...
  await db
    .insertInto('app_users')
    .values(
      [creatorId, queryUserId, operateUserId].map((id) => ({
        id,
    ...
    .execute()
  const creator = { id: creatorId, permissions: [] as string[], trusted: true }
  const enterprise = await service.createBook(
    {
      id: bookIds[0]!,
      name: '企业模板账簿',
      description: '',
      ...
      queryUserIds: [queryUserId],
      operateUserIds: [operateUserId],
    },
    creator,
  )
  ...
  assert.deepEqual(enterprise.queryUserIds, [creatorId, queryUserId].sort())
  assert.deepEqual(enterprise.operateUserIds, [creatorId, operateUserId].sort())
  assert.equal(
    (
      await service.querySubjects(
        { bookId: bookIds[0]!, page: 1, pageSize: 200 },
        creator,
      )
    ).total,
    28,
  ...
  assert.equal(
    (
      await service.querySubjects(
        { bookId: bookIds[1]!, page: 1, pageSize: 200 },
        creator,
      )
    ).total,
    29,
  ...
  assert.equal(
    (
      await service.querySubjects(
        { bookId: bookIds[2]!, page: 1, pageSize: 200 },
        creator,
      )
    ).total,
    0,
  ...
  assert.equal(
    (
      await service.queryBooks(
        { page: 1, pageSize: 20, keyword: '企业模板' },
        {
          id: queryUserId,
          permissions: ['/acc/book/query'],
        },
      )
    ).items[0]?.id,
    bookIds[0],
  ...
  await service.createSubject(
    ...
    { id: operateUserId, permissions: ['/acc/subject/create'] },
  )
})
...
test('ACC Opening persists typed asset, bill, and current customer container facts then reverses them', async (context) => {
  ...
  context.after(async () => {
    try {
      ...
      await sql`DELETE FROM acc_register_entries WHERE opening_approval_entry_id = ${openingId}`.execute(
        db,
      )
      await sql`DELETE FROM acc_bill_book_values WHERE opening_approval_entry_id = ${openingId}`.execute(
        db,
      )
      await sql`DELETE FROM acc_asset_book_values WHERE acquisition_opening_approval_entry_id = ${openingId}`.execute(
        db,
      )
      await sql`DELETE FROM acc_asset_registers WHERE acquisition_opening_approval_entry_id = ${openingId}`.execute(
        db,
      )
      await sql`DELETE FROM acc_bill_registers WHERE created_opening_approval_entry_id = ${openingId}`.execute(
        db,
      )
      await db
        .deleteFrom('approval_events')
      ...
      await db
        ...
        .execute()
      await sql`DELETE FROM dcl_subjects WHERE id IN (${supplierId}, ${customerId})`.execute(
        db,
      )
      await db
        .deleteFrom('acc_mappings')
  ...
  const assetSubject = await service.createSubject(
    {
      id: ulid(),
      bookId,
      code: '1601',
  ...
  const billSubject = await service.createSubject(
    {
      id: ulid(),
      bookId,
      code: '1121',
  ...
  const equitySubject = await service.createSubject(
    {
      id: ulid(),
      bookId,
      code: '4001',
  ...
  await db
    ...
    .values({
      ...
      primary_sales_attribution_approval_entry_id: null,
      primary_sales_attribution_code: null,
      primary_sales_attribution_name: null,
      sales_attribution_snapshot: null,
    ...
    .values({
      ...
      mapping_definition: JSON.stringify({
        assetConfiguration: {
          ...
          assetDimensions: {},
          accumulatedDepreciationSubjectId: equitySubject.id,
          accumulatedDepreciationDimensions: {},
          depreciationExpenseSubjectId: equitySubject.id,
  ...
  assert.equal(
    (
      await sql<{
        count: string
      }>`SELECT count(*)::text AS count FROM acc_asset_book_values WHERE asset_id = ${assetId} AND book_id = ${bookId}`.execute(
        db,
      )
    ).rows[0]!.count,
    '1',
  ...
  assert.equal(
    (
      await sql<{
        count: string
      }>`SELECT count(*)::text AS count FROM acc_bill_book_values WHERE bill_id = ${billId} AND book_id = ${bookId}`.execute(
        db,
      )
    ).rows[0]!.count,
    '1',
  ...
  assert.equal(
    (
      await sql<{
        count: string
      }>`SELECT count(*)::text AS count FROM acc_opening_container_balances WHERE opening_approval_entry_id = ${openingId}`.execute(
        db,
      )
    ).rows[0]!.count,
    '1',
  ...
  await assert.rejects(
    new DclArchiveService(db).review(
      'customer',
      'unapprove',
      {
  ...
  await new VouOpeningService(db, service).reviewOpening(
    ...
    { ...actor, id: reviewerId },
    'opening-typed-unapprove',
  )
  assert.equal(
    (
      await sql<{
        count: string
      }>`SELECT count(*)::text AS count FROM acc_register_entries WHERE opening_approval_entry_id = ${openingId}`.execute(
        db,
      )
    ).rows[0]!.count,
    '0',
  ...
  assert.equal(
    (
      await sql<{
        count: string
      }>`SELECT count(*)::text AS count FROM acc_asset_registers WHERE id = ${assetId}`.execute(
        db,
      )
    ).rows[0]!.count,
    '0',
  ...
  assert.equal(
    (
      await sql<{
        count: string
      }>`SELECT count(*)::text AS count FROM acc_bill_registers WHERE id = ${billId}`.execute(
        db,
      )
    ).rows[0]!.count,
    '0',
...
test('ACC book, subjects, Opening and periods keep one transactional fact boundary', async (context) => {
    ...
    priceLines: [
      {
        product: {
          objectId: productId,
          approvalEntryId: productApprovalEntryId,
  ...
  assert.deepEqual(
    await db
      ...
      .executeTakeFirst(),
    { can_query: true, can_operate: true },
  )
  ...
  `.execute(db)
  assert.deepEqual(
    openingJournal.rows.map((row) => row.source_kind),
    ['OPENING'],
  )
  const openingLines = await db
    .selectFrom('acc_journal_lines')
    .select('id')
    .where('journal_entry_id', '=', openingJournal.rows[0]!.id)
    .execute()
  ...
  assert.equal(
    (
      await sql<{
        count: string
      }>`SELECT count(*)::text AS count FROM acc_register_entries WHERE opening_approval_entry_id = ${submissionId}`.execute(
        db,
      )
    ).rows[0]!.count,
    '0',
  ...
  assert.equal(
    (
      await sql<{
        count: string
      }>`SELECT count(*)::text AS count FROM acc_journal_entries WHERE opening_approval_entry_id = ${submissionId}`.execute(
        db,
      )
    ).rows[0]!.count,
    '0',
  ...
  const reapproved = await new VouOpeningService(db, service).reviewOpening(
    'approve',
    {
      bookId: book.id,
      submissionId,
      expectedRevision: unapprovedOpening.approval.revision,
    },
    reviewer,
    'opening-reapprove',
  )
  ...
  const vouPending = await vou.submit(
    'sale-pricing',
    'submit-new',
    {
      documentId,
      ...
      payload: pricingPayload('25.00'),
    },
    submitter,
    'posting-submit',
  )
  const vouApproved = await vou.review(
    'sale-pricing',
    'approve',
    {
      documentId,
      submissionId: vouSubmissionId,
      expectedRevision: vouPending.revision,
    },
    reviewer,
    'posting-approve',
  )
  ...
  assert.equal(
    (
      await db
        ...
        .select('id')
        .where('vou_approval_entry_id', '=', vouSubmissionId)
        .execute()
    ).length,
    1,
  )
  await vou.review(
    ...
    'unapprove',
    {
      documentId,
      submissionId: vouSubmissionId,
      expectedRevision: vouApproved.revision,
      reason: '撤销记账',
    },
    reviewer,
    'posting-unapprove',
  ...
  const failingVou = new VouService(db, {
    ...
    wfl: {
      async apply() {
        throw new Error('forced WFL failure')
      },
    },
  })
  const failedPending = await failingVou.submit(
    'sale-pricing',
    'submit-new',
    {
  ...
  await assert.rejects(
    failingVou.review(
      ...
      'approve',
      {
        documentId: failedDocumentId,
        submissionId: failedSubmissionId,
        expectedRevision: failedPending.revision,
      },
      reviewer,
      'forced-approve',
    ),
  ...
  assert.equal(
    (await vou.get('sale-pricing', failedDocumentId, reviewer)).status,
    'PENDING',
  )
  assert.equal(
    (
      await db
        .selectFrom('acc_journal_entries')
        .select('id')
        .where('vou_approval_entry_id', '=', failedSubmissionId)
        .execute()
    ).length,
    0,
  )
  const concurrentDocumentId = ulid(),
    concurrentSubmissionId = ulid()
  const concurrentPending = await vou.submit(
    'sale-pricing',
    'submit-new',
    {
      documentId: concurrentDocumentId,
      submissionId: concurrentSubmissionId,
      idempotencyKey: concurrentSubmissionId,
      expectedRevision: null,
      payload: pricingPayload('12.00'),
    },
    submitter,
    'concurrent-submit',
  )
  const attempts = await Promise.allSettled([
    vou.review(
      ...
      'approve',
      {
        documentId: concurrentDocumentId,
        submissionId: concurrentSubmissionId,
        expectedRevision: concurrentPending.revision,
      },
      reviewer,
      'concurrent-1',
    ),
  ...
  assert.equal(
    (
      await db
        .selectFrom('acc_journal_entries')
        .select('id')
        .where('vou_approval_entry_id', '=', concurrentSubmissionId)
        .execute()
    ).length,
    1,
  )
  await assert.rejects(
    new VouOpeningService(db, service).reviewOpening(
      'unapprove',
      {
        bookId: book.id,
        submissionId,
        expectedRevision: reapproved.approval.revision,
        reason: '已有后续记账',
      },
      reviewer,
      'opening-unapprove-blocked',
    ),
    (error: unknown) =>
  ...
  const periodBalances = await sql<{
    subject_id: string
  ...
  assert.deepEqual(periodBalances.rows, [
    {
      ...
      debit_amount: '100.00000000',
      credit_amount: '0.00000000',
      closing_balance: '100.00000000',
    },
    {
      subject_id: credit.id,
      opening_balance: '0.00000000',
      debit_amount: '0.00000000',
      credit_amount: '100.00000000',
      closing_balance: '-100.00000000',
    },
  ])
  const unlocked = await service.setPeriod(
    { bookId: book.id, month: '2026-08', expectedRevision: period.revision },
    false,
    reviewer,
  )
  ...
  assert.equal(
    (
      await sql<{
        count: string
      }>`SELECT count(*)::text AS count FROM acc_period_balances WHERE book_id = ${book.id} AND period_month = '2026-08'`.execute(
        db,
      )
    ).rows[0]!.count,
    '0',
  ...
  context.after(async () => {
    try {
      await db
        ...
        .where('vou_approval_entry_id', '=', vouEntryId)
        .execute()
      await db
        .deleteFrom('acc_journal_entries')
        .where('vou_approval_entry_id', '=', vouEntryId)
      ...
      await db
        .deleteFrom('approval_events')
        .where('entry_id', 'in', [openingId, mappingEntryId, vouEntryId])
        .execute()
      await db
        .deleteFrom('approval_entries')
        .where('id', 'in', [openingId, mappingEntryId, vouEntryId])
        .execute()
      await db
        .deleteFrom('vou_documents')
        .where('id', '=', documentId)
    ...
    db.transaction().execute((tx) =>
      service.apply(tx, {
        ...
        payload: {
          ...
          amount: '10.00',
          quantity: '1',
        } as unknown as import('@zerp/model').VouPayload,
      }),
    ),
    (error: unknown) =>
      error instanceof AccApplicationError &&
      error.errorKey === 'acc_mapping_dimension_required',
  )
  assert.equal(
    (
      await db
...
test('ACC records global asset effects for UN_POST and rejects control-book backdated negative inventory', async (context) => {
  ...
  const assetDocumentId = ulid(),
    ...
    inboundEntryId = ulid(),
    outboundDocumentId = ulid(),
    outboundEntryId = ulid()
  const ids = [assetEntryId, assetSaleEntryId, inboundEntryId, outboundEntryId]
  context.after(async () => {
    try {
      await db
        .deleteFrom('acc_inventory_entries')
        .where('vou_approval_entry_id', 'in', ids)
        .execute()
      await db
        .deleteFrom('acc_journal_entries')
        .where('vou_approval_entry_id', 'in', ids)
      ...
      await db
        ...
        .where('vou_approval_entry_id', 'in', ids)
        .execute()
      await sql`DELETE FROM acc_asset_book_values WHERE acquisition_vou_approval_entry_id = ${assetEntryId}`.execute(
        db,
      )
      await sql`DELETE FROM acc_asset_registers WHERE acquisition_vou_approval_entry_id = ${assetEntryId}`.execute(
        db,
      )
      await db
        .deleteFrom('approval_events')
        .where('entry_id', 'in', [openingId, mappingEntryId, ...ids])
        .execute()
      await db
        .deleteFrom('approval_entries')
        .where('id', 'in', [openingId, mappingEntryId, ...ids])
        .execute()
      await db
        .deleteFrom('vou_documents')
        .where('id', 'in', [
  ...
  const mapping = (
    ...
    db
      ...
      .values({
        ...
        mapping_definition: JSON.stringify(definition),
      })
      .execute()
  await mapping(
    'asset-acquisition',
  ...
  const createVou = async (
    documentId: string,
    entryId: string,
    entity: 'asset-acquisition' | 'asset-sale' | 'sale-pricing',
    documentNo: string,
  ) => {
    await db
      .insertInto('vou_documents')
  ...
  assert.equal(
    (
      await sql<{
        count: string
      }>`SELECT count(*)::text AS count FROM acc_asset_registers WHERE acquisition_vou_approval_entry_id = ${assetEntryId}`.execute(
        db,
      )
    ).rows[0]!.count,
    '1',
  ...
  assert.equal(
    (
      await db
        .selectFrom('acc_journal_entries')
        .select('id')
        .where('vou_approval_entry_id', '=', assetEntryId)
        .execute()
    ).length,
    0,
  )
  const assetId = (
    await sql<{
      id: string
    }>`SELECT id FROM acc_asset_registers WHERE acquisition_vou_approval_entry_id = ${assetEntryId}`.execute(
      db,
    )
  ).rows[0]!.id
  ...
  await db.transaction().execute((tx) =>
    service.apply(tx, {
      ...
      action: 'approve',
      entity: 'asset-sale',
      documentId: assetSaleDocumentId,
      documentNo: 'DSL-TEST',
      approvalEntryId: assetSaleEntryId,
      approvalRevision: '1',
      occurredAt: now.toISOString(),
      payload: {
        ...
        assetSaleLines: [{ assetId, saleAmount: '120.00' }],
      },
    }),
  )
  assert.equal(
    (
      await sql<{
        status: string
      }>`SELECT status FROM acc_asset_registers WHERE id = ${assetId}`.execute(
        db,
      )
    ).rows[0]!.status,
    'SOLD',
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof AccApplicationError &&
      error.errorKey === 'acc_register_unapprove_blocked',
  )
  await db.transaction().execute((tx) =>
    service.apply(tx, {
      kind: 'acc',
  ...
  await db
    ...
    .set({
      ...
      mapping_definition: JSON.stringify({
        ...
        rules: [
          {
            conditions: [{ field: 'mode', operator: 'EQ', values: ['OUT'] }],
            result: 'POST',
            templateId: 'outbound',
          },
        ],
  ...
  await db.transaction().execute((tx) =>
    service.apply(tx, {
      ...
      documentNo: 'SPR-IN',
      approvalEntryId: inboundEntryId,
      approvalRevision: '1',
      occurredAt: now.toISOString(),
      payload: {
        businessDate: '2026-09-05',
  ...
  assert.equal(
    (
      await db
        .selectFrom('acc_inventory_entries')
        .select('id')
        .where('vou_approval_entry_id', '=', inboundEntryId)
        .execute()
    ).length,
    1,
  )
  ...
  await assert.rejects(
    db.transaction().execute((tx) =>
      service.apply(tx, {
        ...
        action: 'approve',
        entity: 'sale-pricing',
        documentId: outboundDocumentId,
        documentNo: 'SPR-OUT',
        approvalEntryId: outboundEntryId,
        approvalRevision: '1',
        occurredAt: now.toISOString(),
        payload: {
          businessDate: '2026-09-04',
  ...
  assert.equal(
    (
      await db
        .selectFrom('acc_inventory_entries')
        .select('id')
        .where('vou_approval_entry_id', '=', outboundEntryId)
        .execute()
    ).length,
    0,
  )
...
test('ACC records and exactly reverses sale-signoff empty-container deltas without JSON fallback', async (context) => {
  ...
  context.after(async () => {
    try {
      await sql`DELETE FROM acc_container_entries WHERE vou_approval_entry_id = ${approvalEntryId}`.execute(
        db,
      )
      await db
        .deleteFrom('approval_events')
  ...
  await db
    .transaction()
    .execute((tx) => service.apply(tx, { ...plan, action: 'unapprove' }))
  assert.equal(
    (
