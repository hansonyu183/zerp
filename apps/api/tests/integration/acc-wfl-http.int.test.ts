...
async function seedPermissions(
  ...
  const created = paths
    ...
    .map((path) => ({ id: ulid(), path, ...permissionParts(path) }))
  if (created.length > 0)
    await db
...
async function createPrincipal(
  ...
  const id = ulid(),
    roleId = ulid(),
    password = `Target!${randomBytes(18).toString('base64url')}`
...
async function seedSaleOrderReferences(
  ...
  const submit = async (
    ...
    if (pendingResponse)
      assert.equal(
        pendingResponse.code,
        0,
        `${entity} submit: ${pendingResponse.errorKey}`,
      )
    ...
    const approvedResponse = crossesHttp
      ? await post(
          origin,
          reviewerSession,
          `/dcl/${entity}/approve`,
    ...
    if (approvedResponse)
      assert.equal(
        approvedResponse.code,
        0,
        `${entity} approve: ${approvedResponse.errorKey}`,
      )
    ...
    if (crossesHttp) {
      ...
      assert.deepEqual(readback.data.snapshot, approved.snapshot)
    }
    return {
      objectId,
      approvalEntryId,
...
test('ACC HTTP routes enforce session, CSRF, permissions, envelope, opening and period facts', async (context) => {
  ...
  const submitter = await createPrincipal(
    db,
    'acc-submit',
    permissionFixture.ids,
  )
  ...
  const created = await post(origin, submitterSession, '/acc/book/create', {
    id: bookId,
    name: 'HTTP 账簿',
    description: '',
    startMonth: '2026-08',
  ...
  assert.equal(
    (await post(origin, submitterSession, '/acc/book/query', {})).data.items[0]
      .id,
    bookId,
  )
  ...
  const periods = await post(origin, reviewerSession, '/acc/period/query', {
    bookId,
  })
  assert.equal(periods.data[0].month, '2026-08')
  const unlocked = await post(origin, reviewerSession, '/acc/period/unlock', {
    bookId,
    month: '2026-08',
    expectedRevision: locked.data.revision,
  })
