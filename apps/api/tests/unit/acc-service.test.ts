...
test('ACC restores the approved f856118f accounting subject template catalogs', () => {
  ...
  assert.deepEqual(
    accSubjectTemplates.ENTERPRISE.find((line) => line.code === '1122'),
    {
      code: '1122',
      name: '应收账款',
      parentCode: '1000',
  ...
  assert.deepEqual(
    accSubjectTemplates.SMALL_BUSINESS.find((line) => line.code === '3001'),
    {
      code: '3001',
      name: '实收资本',
      parentCode: '3000',
...
test('ACC enforces settlement and inventory dimensions before persistence', () => {
  ...
  assert.throws(
    ...
    {
      name: 'AccApplicationError',
      errorKey: 'acc_subject_settlement_dimension_required',
    },
  )
  assert.throws(
    () =>
    ...
    {
      name: 'AccApplicationError',
      errorKey: 'acc_subject_inventory_dimension_required',
    },
  )
  assert.throws(
    () =>
