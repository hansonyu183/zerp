...
test('migrated people expose no old DCL lifecycle or BOB current routes', async () => {
  const app = createApp({
    ...
    config: loadConfig({
      DATABASE_URL: 'postgres://unused:unused@127.0.0.1:5432/zerp_test',
    }),
  })
  assert.notEqual(
    (
  ...
  for (const entity of ['employee', 'operating-entity']) {
    for (const action of [
      ...
      const response = await app.request(`/dcl/${entity}/${action}`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: '{}',
      })
    ...
    for (const action of ['query', 'get']) {
      const response = await app.request(`/bob/${entity}/${action}`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: '{}',
      })
