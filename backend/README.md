...
`make seed-preview` 允许在 `development` 或隔离的 `test` 环境运行，补齐 AUX、BOB、全部 VOU
单据、WFL、ACC 和预置 RPT 所需事实。每个业务对象或单据至少提供可操作草稿或关键已批准链路，
并建立控制账簿、零期初及全单据会计映射；重复执行只恢复 seed 自身中断的步骤，不覆盖测试人员
已经修改或推进的样本。
