...
CREATE TABLE dcl_vehicle_versions (
  ...
  category_id character varying(26), category_approval_entry_id character varying(26),
  category_entity character varying(16) NOT NULL DEFAULT 'category' CHECK (category_entity='category'),
  ...
  FOREIGN KEY (category_id,category_entity) REFERENCES aux_objects(id,entity) ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED,
...
CREATE INDEX dcl_vehicle_versions_category_idx ON dcl_vehicle_versions(category_id);
...
SELECT version.approval_entry_id,version.entity,version.name,version.plate_number,version.vehicle_type,
  version.category_id,version.category_approval_entry_id,version.category_entity,version.vin,version.engine_number,
...
SELECT object.id,entry.id,version.name,version.plate_number,version.vehicle_type,version.vin,version.engine_number,version.load_capacity_kg,version.remark,
...
JOIN bob_vehicle_versions version ON version.approval_entry_id=entry.id WHERE object.entity='vehicle';
