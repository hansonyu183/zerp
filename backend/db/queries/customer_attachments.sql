-- name: LockCustomerAttachmentGroup :one
SELECT id,revision FROM bob_customer_groups WHERE id=sqlc.arg(owner_id) FOR UPDATE;
...
-- name: CountCustomerGroupAttachments :one
SELECT count(*) FROM bob_customer_group_attachments WHERE group_id=sqlc.arg(owner_id);
...
-- name: InsertCustomerGroupAttachment :exec
...
-- name: TouchCustomerGroupAttachment :one
UPDATE bob_customer_groups
SET revision=revision+1,updated_at=now(),updated_by=sqlc.arg(actor_id)
WHERE id=sqlc.arg(owner_id) AND revision=sqlc.arg(revision)
RETURNING revision;

-- name: InsertCustomerGroupAttachmentAudit :exec
INSERT INTO bob_customer_group_audit_events(
    id,group_id,event_type,actor_id,request_id,summary
) VALUES (
    sqlc.arg(id),sqlc.arg(group_id),sqlc.arg(event_type),sqlc.arg(actor_id),
    sqlc.arg(request_id),sqlc.arg(summary)
);

-- name: TouchCustomerVersionAttachment :one
UPDATE bob_versions
SET revision=revision+1,updated_at=now(),updated_by=sqlc.arg(actor_id)
WHERE id=sqlc.arg(owner_id) AND entity='customer' AND status='DRAFT' AND revision=sqlc.arg(revision)
RETURNING revision;

-- name: ListCustomerGroupAttachments :many
...
WHERE relation.group_id=sqlc.arg(owner_id)
...
-- name: GetReadyCustomerGroupAttachment :one
SELECT file.id,file.storage_key,file.original_name,file.content_type,file.declared_size
FROM bob_customer_group_attachments relation
JOIN bob_customer_files file ON file.id=relation.file_id
WHERE relation.group_id=sqlc.arg(owner_id) AND file.id=sqlc.arg(file_id) AND file.status='READY';
...
-- name: DeleteCustomerGroupAttachment :execrows
DELETE FROM bob_customer_group_attachments
WHERE group_id=sqlc.arg(owner_id) AND file_id=sqlc.arg(file_id);
...
SELECT (SELECT count(*) FROM bob_customer_group_attachments group_relation WHERE group_relation.file_id=sqlc.arg(target_file_id))+
