...
LEFT JOIN dcl_rpt_definition_versions v ON v.approval_entry_id=e.id AND v.definition_id=d.id WHERE d.id=sqlc.arg(definition_id);
...
UPDATE dcl_rpt_definition_versions SET validity='INVALID', invalidated_at=now(), invalid_reason='STRUCTURE_CHANGED', updated_at=now(), updated_by=sqlc.arg(actor_id) WHERE approval_entry_id=sqlc.arg(approval_entry_id) AND validity='VALID';
