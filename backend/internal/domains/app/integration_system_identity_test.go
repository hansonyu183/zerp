...
func TestSystemIdentityIntegration(t *testing.T) {
	...
	user, err := service.GetUserDetail(t.Context(), systemidentity.UserID, integrationPrincipal(admin.ID))
	if err != nil {
		t.Fatalf("get system user: %v", err)
	}
	userRoles := userRoleIDs(user)
	if user.Username != systemidentity.Username || user.DisplayName != systemidentity.UserDisplayName ||
		user.Status != StatusDisabled || len(userRoles) != 1 || userRoles[0] != systemidentity.RoleID {
		t.Fatalf("system user = %+v", user)
	}

	role, err := service.GetRole(t.Context(), systemidentity.RoleID, integrationPrincipal(admin.ID))
	if err != nil {
		t.Fatalf("get system role: %v", err)
	}
	if role.Code != systemidentity.RoleCode || role.Name != systemidentity.RoleName ||
		role.Status != StatusEnabled || len(rolePermissionIDs(role)) != 0 {
	...
	}, integrationPrincipal(admin.ID), "create-system-user"); !errorIsKind(err, ErrorConflict) {
	...
	}, integrationPrincipal(admin.ID), "create-system-role"); !errorIsKind(err, ErrorValidation) {
		t.Fatalf("create system role error = %v", err)
	}
	if _, err = service.SaveUser(t.Context(), SaveUserInput{
		ID: user.ID, DisplayName: "changed", RoleIDs: userRoles, Revision: user.Revision,
	}, integrationPrincipal(admin.ID), "save-system-user"); !errorIsKind(err, ErrorConflict) {
		t.Fatalf("save system user error = %v", err)
	}
	if _, err = service.SetUserStatus(t.Context(), user.ID, user.Revision, StatusEnabled, integrationPrincipal(admin.ID), "enable-system-user"); !errorIsKind(err, ErrorConflict) {
	...
	}, integrationPrincipal(admin.ID), "save-system-role"); !errorIsKind(err, ErrorForbidden) {
		t.Fatalf("save system role error = %v", err)
	}
	if _, err = service.SetRoleStatus(t.Context(), role.ID, role.Revision, StatusDisabled, integrationPrincipal(admin.ID), "disable-system-role"); !errorIsKind(err, ErrorForbidden) {
	...
	}, integrationPrincipal(admin.ID), "assign-system-role"); !errorIsKind(err, ErrorConflict) {
