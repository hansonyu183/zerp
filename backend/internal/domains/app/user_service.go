...
func validateUserQuery(request PageRequest) (pageSpec, *string, *string, error) {
	if request.Page < 1 || request.PageSize != 20 || len(request.Sort) != 1 ||
		...
		return pageSpec{}, nil, nil, domainError(ErrorValidation, "invalid user query pagination or sort", nil)
	}
	...
	return spec, status, search, nil
}

func (s *Service) QueryUsers(ctx context.Context, request PageRequest, principal Principal) (Page[UserListItem], error) {
	spec, status, search, err := validateUserQuery(request)
	if err != nil {
		return Page[UserListItem]{}, err
	}
	tx, err := s.pool.Begin(ctx)
	if err != nil {
		return Page[UserListItem]{}, s.internal("begin user query", err)
	}
	defer tx.Rollback(ctx) //nolint:errcheck
	qtx := s.queries.WithTx(tx)
	if err = qtx.AcquireAppAuthorizationLock(ctx); err != nil {
		return Page[UserListItem]{}, s.internal("lock user query", err)
	}
	actor, err := s.currentActorAuthorization(ctx, qtx, principal)
	if err != nil {
		return Page[UserListItem]{}, err
	}
	if err = actor.require("/app/user/query"); err != nil {
		return Page[UserListItem]{}, err
	}
	total, err := qtx.CountAppUsers(ctx, dbsqlc.CountAppUsersParams{Status: status, Search: search})
	if err != nil {
		return Page[UserListItem]{}, s.internal("count users", err)
	}
	rows, err := qtx.ListAppUsers(ctx, dbsqlc.ListAppUsersParams{Status: status, Search: search, SortField: spec.SortField, SortOrder: spec.SortOrder, PageOffset: spec.Offset, PageSize: int32(spec.PageSize)})
	if err != nil {
		return Page[UserListItem]{}, s.internal("list users", err)
	}
	items := make([]UserListItem, 0, len(rows))
	for _, row := range rows {
		user := userListView(row)
		manageable, manageErr := s.userManageable(ctx, qtx, user, actor)
		if manageErr != nil {
			return Page[UserListItem]{}, manageErr
		}
	...
	if err = tx.Commit(ctx); err != nil {
		return Page[UserListItem]{}, s.internal("commit user query", err)
	}
	return Page[UserListItem]{Items: items, Total: total, Page: spec.Page, PageSize: spec.PageSize}, nil
}
...
func (s *Service) CreateUser(ctx context.Context, input CreateUserInput, principal Principal, requestID string) (UserDetail, error) {
...
func (s *Service) SaveUser(ctx context.Context, input SaveUserInput, principal Principal, requestID string) (UserDetail, error) {
	input.DisplayName = strings.TrimSpace(input.DisplayName)
	input.RoleIDs = uniqueStrings(input.RoleIDs)
...
func (s *Service) SetUserStatus(ctx context.Context, id string, revision int64, status string, principal Principal, requestID string) (UserDetail, error) {
...
func (s *Service) ResetUserPassword(ctx context.Context, input ResetPasswordInput, principal Principal, requestID string) (ResetPasswordResult, error) {
	...
	if input.ID == actor.id || systemidentity.IsUser(input.ID) {
