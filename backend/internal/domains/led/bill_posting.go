...
func (s *Service) postBillReceipt(ctx context.Context, tx pgx.Tx, q *dbsqlc.Queries, p postingContext) error {
	...
	for _, l := range lines {
		if l.Purpose == "CHANGE" {
			...
			balance, balanceErr := q.GetLedBillAvailableBalance(ctx, dbsqlc.GetLedBillAvailableBalanceParams{GenerationID: p.GenerationID, BillID: b.ID, PositionType: l.PositionType, AsOfDate: doc.BusinessDate})
...
func (s *Service) postBillPayment(ctx context.Context, tx pgx.Tx, q *dbsqlc.Queries, p postingContext) error {
	...
	for _, line := range lines {
		...
		balance, balanceErr := q.GetLedBillAvailableBalance(ctx, dbsqlc.GetLedBillAvailableBalanceParams{
			GenerationID: p.GenerationID, BillID: bill.ID, PositionType: "ASSET", AsOfDate: doc.BusinessDate,
		})
		if balanceErr != nil || balance != 1 {
			return domainError(ErrorConflict, "source bill is not available", nil, balanceErr)
		}
...
func (s *Service) postBillDiscount(ctx context.Context, tx pgx.Tx, q *dbsqlc.Queries, p postingContext) error {
	...
	for _, line := range lines {
		...
		balance, balanceErr := q.GetLedBillAvailableBalance(ctx, dbsqlc.GetLedBillAvailableBalanceParams{GenerationID: p.GenerationID, BillID: bill.ID, PositionType: "ASSET", AsOfDate: doc.BusinessDate})
		if balanceErr != nil || balance != 1 {
			return domainError(ErrorConflict, "source bill is not available", nil, balanceErr)
		}
...
func (s *Service) postBillMaturity(ctx context.Context, tx pgx.Tx, q *dbsqlc.Queries, p postingContext) error {
	...
	for _, line := range lines {
		...
		balance, balanceErr := q.GetLedBillAvailableBalance(ctx, dbsqlc.GetLedBillAvailableBalanceParams{GenerationID: p.GenerationID, BillID: bill.ID, PositionType: position, AsOfDate: doc.BusinessDate})
