...
func (s *Service) prepareAssetDraft(ctx context.Context, tx pgx.Tx, q *dbsqlc.Queries, entity string, input DraftInput, saved *DocumentDataView) (preparedAssetDraft, error) {
	...
	case EntityAssetSale:
		if input.CounterpartyType != "customer-subunit" && input.CounterpartyType != "other-unit" {
			return result, domainError(ErrorValidation, "counterpartyType must be customer-subunit or other-unit", nil, nil)
		}
