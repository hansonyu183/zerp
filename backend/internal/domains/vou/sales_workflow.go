...
func (s *Service) linkSalesWorkflowDocument(
	...
	if err := tx.QueryRow(ctx, `SELECT process_id FROM wfl_process_documents
		WHERE document_id=$1`, parentID).Scan(&processID); errors.Is(err, pgx.ErrNoRows) {
		return nil
	} else if err != nil {
...
func (s *Service) onManagedSalesApproved(
	...
	var composed bool
	if err := tx.QueryRow(ctx, `SELECT EXISTS(
		SELECT 1 FROM wfl_process_documents WHERE document_id=$1
	)`, document.ID).Scan(&composed); err != nil {
		return nil, s.internal("check sales workflow composition", err)
	}
	if !composed {
		return nil, nil
	}
...
func (s *Service) ensureAutoOutboundDraft(
	...
	if err = s.finishAutoSalesDocument(
		...
		map[string]any{"parentDocumentId": orderID, "parentDocumentNo": orderNumber},
	); err != nil {
		return MutationResult{}, err
	}
...
func (s *Service) insertAutoSalesDocument(
	...
	_, err = tx.Exec(ctx, `INSERT INTO vou_documents(
		...
		id, entity, number, salesParentEntity(entity), parentID, date, currency, total, actorID)
	if err != nil {
...
func (s *Service) finishAutoSalesDocument(
	...
	return s.events.Publish(ctx, tx, DocumentCreatedEvent{
		Entity: entity, DocumentID: documentID, DocumentNo: documentNo, Revision: 1,
		ParentEntity: salesParentEntity(entity), ParentDocumentID: parentID,
...
func (s *Service) ensureAutoDeliveryDraft(
	...
	err := tx.QueryRow(ctx, `SELECT d.document_no,d.status,d.business_date,d.currency,d.total_amount_cents,
		...
		WHERE d.id=$1 FOR UPDATE OF d`, outboundID).Scan(
	...
	if err = s.finishAutoSalesDocument(
		...
		map[string]any{"parentDocumentId": outboundID, "parentDocumentNo": sourceNumber},
	); err != nil {
		return MutationResult{}, err
	}
	return MutationResult{DocumentID: id, DocumentNo: number, Status: StatusDraft, Revision: 1}, nil
...
func (s *Service) ensureAutoSignoffDraft(
	...
	err := tx.QueryRow(ctx, `SELECT d.document_no,d.status,d.business_date,d.currency,
		...
		WHERE d.id=$1 FOR UPDATE OF d`, deliveryID).Scan(
	...
	if err = s.finishAutoSalesDocument(
		...
		map[string]any{"parentDocumentId": deliveryID, "parentDocumentNo": sourceNumber},
	); err != nil {
		return MutationResult{}, err
	}
	return MutationResult{DocumentID: id, DocumentNo: number, Status: StatusDraft, Revision: 1}, nil
...
func (s *Service) deleteGeneratedSalesDocument(
	...
	case EntityEmployeeLoanWriteoff:
		if _, err := tx.Exec(ctx, `DELETE FROM vou_expense_lines WHERE document_id=$1;
			DELETE FROM vou_employee_loan_writeoff_details WHERE document_id=$1`, documentID); err != nil {
			return err
		}
	default:
