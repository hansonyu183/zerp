...
case "${input}" in
  ...
  *issue-316-wfl-identity-and-dcl-code*)
    ...
    [ "${count}" -ne 1 ] || exit 1
    ;;
  *issue-318-rpt-counter-and-terminology*)
    count_file="${MOCK_DOCKER_STATE}.issue-318"
    count=0
    [ ! -f "${count_file}" ] || count=$(cat "${count_file}")
    count=$((count + 1))
    printf '%s\n' "${count}" >"${count_file}"
...
test "$(cat "${tmp}/docker-state.issue-318")" = 2
