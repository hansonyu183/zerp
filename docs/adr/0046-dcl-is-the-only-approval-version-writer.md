...
date: 2026-08-29
status: accepted
partially_superseded_by: ADR-0051, ADR-0055, ADR-0056, ADR-0057
---
...
切换后的权威结构不保留旧表、旧路由、旧权限、旧事件、旧错误语义、旧维护页面、双写、兼容读取或 fallback。生产代码、契约、生成物和 seed 中的旧入口必须删除；测试中只允许保留明确证明旧入口未注册、旧表不存在或旧协议不可用的墓碑断言。数据库 schema、Hono 契约和领域文档只描述当前结构。

最终验收同时覆盖 Hono OpenAPI/客户端生成、Kysely schema 校验、API 单元测试和真实 PostgreSQL 集成、前端类型检查/单元测试/构建、串行真实 E2E、文档检查及旧标识精确搜索。任一消费领域重新出现本地版本写入、候选生命周期或旧协议，即违反本 ADR。

## 切片验收记录

| 切片       | 最终写边界                                                                 | 数据与契约证据                                                                                                                  | 运行与墓碑证据                                                                                             |
| ---------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| #278–#282  | 经营主体、仓库、车辆、资金账户、产品版本由 DCL 写入，BOB 只保留 current    | DCL/BOB 生成契约与 `dcl_*_versions`                                                                                             | 各 DCL PostgreSQL lifecycle/current integration 与 BOB 旧写路由墓碑测试                                    |
...
| #294       | 全仓只有 DCL 写入 Approval Version                                         | Hono 契约与 Kysely 类型重新生成无漂移；权威 schema 只包含 DCL 版本 payload                                                      | API 单元/真实 PostgreSQL、前端 unit/build/E2E、文档/schema gate 与精确 tombstone 搜索                      |
...
| #344、#351 | Customer 单一聚合拥有身份、资料及全部客户子单位                            | `/dcl/customer` 与 `dcl_customer_versions` 保存唯一候选；子单位只保存 Customer 内 stable ID 与版本行                            | 分命令 revision、候选隔离、历史子单位精确引用、旧路径墓碑与销售收款多子单位集成                            |
| #345–#347  | 五类身份档案直接拥有 typed snapshot，BOB 只保留内部 current/reference 读取 | Party、relationship、claim、merge、独立 Customer Subunit subject 及重复 BOB 页面全部删除；Hono 契约与 Kysely 类型重新生成       | DCL 唯一维护入口、旧深链不可达、真实 PostgreSQL 重置播种与真实 API E2E                                     |

本 ADR 收束 [ADR-0032](0032-central-approval-persistence-and-lifecycle.md) 建立的中央 Approval 边界，以及 [ADR-0033](0033-dcl-operating-entity-declaration-and-bob-read-boundary.md) 至 [ADR-0045](0045-wfl-process-definition-declarations-are-dcl-owned.md) 的逐域切换。对应 GitHub [#293](https://github.com/hansonyu183/zerp/issues/293) 与 [#294](https://github.com/hansonyu183/zerp/issues/294)；父任务见 [#276](https://github.com/hansonyu183/zerp/issues/276)。
