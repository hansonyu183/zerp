...
| #345–#347  | 五类身份档案直接拥有 typed snapshot，BOB 只保留内部 current/reference 读取 | Party、relationship、claim、merge、独立 Customer Subunit subject 及重复 BOB 页面全部删除；OpenAPI 与 sqlc 重新生成              | DCL 唯一维护入口、旧深链不可达、真实 PostgreSQL 重置播种与真实 API E2E                                     |
