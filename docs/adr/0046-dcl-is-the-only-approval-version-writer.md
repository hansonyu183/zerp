...
| #344      | Customer 单一聚合拥有身份、资料及全部核算账户                              | `/dcl/customer` 与 `dcl_customer_versions` 保存唯一候选；账户只保存 Customer 内 stable ID 与版本行                              | 聚合事务、候选隔离、历史账户精确引用、旧账户路由/权限/工作台墓碑与销售收款多账户集成                       |
| #345–#347 | 五类身份档案直接拥有 typed snapshot，BOB 只保留内部 current/reference 读取 | Party、relationship、claim、merge、独立 Customer Account subject 及重复 BOB 页面全部删除；OpenAPI 与 sqlc 重新生成              | DCL 唯一维护入口、旧深链不可达、真实 PostgreSQL 重置播种与真实 API E2E                                     |
