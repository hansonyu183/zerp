...
`dcl_subjects.code` 在数据库和查询边界保持 nullable 持久化事实。DCL、BOB、APP 和 RPT 的查询直接读取该 nullable 值；要求业务编码的 Domain consumer 在消费处拒绝缺失事实并返回应用数据不变量错误。任何消费方都不得把缺失值转换为空字符串、占位编码或 `COALESCE` 结果，也不得静默过滤掉该 subject。ACC Mapping 是允许无编码的例外，支持它的消费方必须保留空值语义。
