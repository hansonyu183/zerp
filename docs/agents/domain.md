...
- **`apps/api/`** when the task involves the HTTP wire contract generated from executable Hono/Zod routes.
...
Do not introduce per-module `CONTEXT.md` files or duplicate domain documentation under `frontend/` or `apps/api/`.
...
`docs/domains/` is the source of truth for business rules. Executable Hono/Zod routes under `apps/api/` own the HTTP contract. `docs/use-cases/` owns page-oriented orchestration and acceptance flows, and must link to rather than restate domain rules or wire schemas. `CONTEXT.md` supplies shared vocabulary; it does not replace any of these sources.
