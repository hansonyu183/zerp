...
管理动作资格包含 `edit`、`enable`、`disable`、`delete`，中文分别为编辑、启用、停用、删除，由服务端结合精确权限和对象事实提供；布尔启用事实显示为启用或停用。动作资格不是执行授权，执行时重新检查。创建不接受服务端身份、编码、拼音、启用事实或 revision；保存只改变 typed data，不能夹带启停。管理输入不接受旧身份别名或数字 revision，所有 revision 运算保持大整数精度。引用候选与历史嵌入快照保留自身身份语义。
