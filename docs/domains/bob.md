...
本文定义 ZERP **BOB（Business Object）** 领域的业务模型、状态机、数据约束和事务边界，覆盖经营主体、仓库与车辆当前业务投影，以及客户、供应商、员工、产品和资金账户等 BOB 自有业务对象。经营主体、仓库与车辆申报由 DCL 管理；产品类型、产品分类、部门、岗位、计量单位、字典、结算方式和收支类型由 AUX 领域管理。HTTP 路径和数据结构以根目录 OpenAPI 为准。
