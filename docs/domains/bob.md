...
本文定义 ZERP **BOB（Business Object）** 领域的业务模型、状态机、数据约束和事务边界，覆盖经营主体、仓库、车辆与资金账户当前业务投影，以及客户、供应商、员工和产品等 BOB 自有业务对象。经营主体、仓库、车辆与资金账户申报由 DCL 管理；产品类型、产品分类、部门、岗位、计量单位、字典、结算方式和收支类型由 AUX 领域管理。HTTP 路径和数据结构以根目录 OpenAPI 为准。
...
BOB 列表必须一次返回稳定对象、最新批准版本摘要和开放候选版本摘要；员工主体身份、产品单位换算和产品配方等列表补充数据使用固定次数的批量查询，查询次数不得随列表行数增长。详情读取可以按对象执行实体专属查询。
