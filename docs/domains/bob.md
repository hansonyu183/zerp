...
本文定义 ZERP **BOB（Business Object）** 领域的业务模型、状态机、数据约束和事务边界，覆盖经营主体与仓库当前业务投影，以及客户、供应商、员工、产品、车辆和资金账户等 BOB 自有业务对象。经营主体与仓库申报由 DCL 管理；产品类型、产品分类、部门、岗位、计量单位、字典、结算方式和收支类型由 AUX 领域管理。HTTP 路径和数据结构以根目录 OpenAPI 为准。
...
- 保存经营主体与仓库批准结果的当前业务投影，并提供当前与精确历史版本引用解析；
- 建立个人或组织的稳定主体身份；
...
BOB 自有实体共享版本创建、提交、撤回、审核、驳回、启停、revision 和审计等真正一致的生命周期规则；实体专属的查询投影、明细补充、保存校验和引用解析由对应实体实现负责。经营主体与仓库生命周期固定由 DCL 编排，不进入 BOB 通用写服务。新增实体字段或查询规则不得继续堆入一个要求理解全部 BOB 实体的万能查询或保存流程，也不得为了消除表面重复而把实体规则改造成运行时 metadata。
...
车辆候选变更车型、承运归属、载重或散水承运能力时，最新已批准车辆版本继续供新送货使用；候选审核通过后一次切换。安全、资格或承运责任发生紧急变化时显式禁用车辆，不用候选变更暗示立即停运。
...
BOB 自有对象的 `enabled` 仍属于稳定对象。DCL 所有的经营主体与仓库则把 `enabled` 作为完整申报快照字段，只有批准或反批时才同步改变 BOB current；禁用后不可用于新的业务引用，历史快照不受影响。
...
14. 外部服务关系编辑期间原 latest approved 仍可被相关车辆引用，候选审核通过后自动成为新的 latest approved；任一承运归属对象显式禁用后新业务引用被拒绝；
