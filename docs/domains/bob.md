...
本文定义 ZERP **BOB（Business Object）** 领域的业务模型、状态机、数据约束和事务边界，覆盖经营主体当前业务投影，以及客户、供应商、员工、产品、仓库、车辆和资金账户等 BOB 自有业务对象。经营主体申报由 DCL 管理；产品类型、产品分类、部门、岗位、计量单位、字典、结算方式和收支类型由 AUX 领域管理。HTTP 路径和数据结构以根目录 OpenAPI 为准。
...
- 保存经营主体批准结果的当前业务投影，并提供当前与精确历史版本引用解析；
- 建立个人或组织的稳定主体身份；
...
BOB 自有实体共享版本创建、提交、撤回、审核、驳回、启停、revision 和审计等真正一致的生命周期规则；实体专属的查询投影、明细补充、保存校验和引用解析由对应实体实现负责。经营主体生命周期固定由 DCL 编排，不进入 BOB 通用写服务。新增实体字段或查询规则不得继续堆入一个要求理解全部 BOB 实体的万能查询或保存流程，也不得为了消除表面重复而把实体规则改造成运行时 metadata。
...
`operating-entity`（经营主体）表示我方实际承担合同销售方、开票方和收款方责任的法人公司，不是商品品牌、客户类型或报表标签。DCL 拥有它的稳定申报、强类型快照和候选编排，中央 Approval 拥有版本与审批事实；BOB 维护服务端编码、批准后的当前业务投影和交易引用。页面仍位于 BOB 导航，但全部写动作使用 DCL API，BOB 只保留当前 `query/get`。完整规则见 [DCL 经营主体申报](dcl.md)。每个我方资金账户必须且只能属于一个当前可用经营主体，一个经营主体可以拥有多个资金账户。
...
仓库采用相同的连续可用规则：编辑 latest approved 仓库时，旧批准版本继续供出入库、生产、退货和盘点引用，候选版本审核通过后自动成为新的 latest approved。需要立即阻止新的库存业务时显式禁用仓库；创建候选版本不得造成全局库存业务中断。
...
仓库停用不提供独立预检。用户确认后直接调用停用接口；接口在同一事务内锁定仓库对象、已批准版本、库存和相关单据，并一次校验：业务控制账簿中该仓库所有产品的实时库存均为零，不存在引用该仓库的 `DRAFT` 或 `PENDING` VOU，不存在仍可生成后续库存动作的已批准来源单据，也不存在当前已批准 BOB 引用。任一阻断存在时返回 `inventory`、`documents`、`sources`、`references` 四类最小冲突摘要，仓库保持启用且不产生部分状态变化。已批准历史库存事实和快照不阻止停用，也不被改写。
