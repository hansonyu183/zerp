...
Party stable root 只保存永久 ID 与合并状态；共享身份的候选、版本、审批和审计由 DCL 承载，BOB 仅保存 latest approved 的 current projection。客户、供应、雇佣、服务和销售合作关系各自使用版本、审核和启停生命周期；停用一条关系不得影响同一主体的其他关系。主体作为永久身份保留，业务退出完全由各关系停用表达；误建重复主体只通过主体合并处理。
