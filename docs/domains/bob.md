...
BOB 自有实体共享版本创建、提交、撤回、审核、驳回、启停、revision 和审计等真正一致的生命周期规则；实体专属的查询投影、明细补充、保存校验和引用解析由对应实体实现负责。经营主体、仓库、车辆、资金账户、产品、员工、其他单位与销售合作方生命周期固定由 DCL 编排，不进入 BOB 通用写服务。新增实体字段或查询规则不得继续堆入一个要求理解全部 BOB 实体的万能查询或保存流程，也不得为了消除表面重复而把实体规则改造成运行时 metadata。
...
Party stable root 只保存永久 ID 与合并状态；共享身份的候选、版本、审批和审计由 DCL 承载，BOB 仅保存 latest approved 的 current projection。客户与供应关系各自使用 BOB 生命周期；雇佣、服务和销售合作关系的候选、启停和审批分别由 DCL employee、other-unit 与 sales-partner 承载。停用一条关系不得影响同一主体的其他关系。主体作为永久身份保留，业务退出完全由各关系停用表达；误建重复主体只通过主体合并处理。

主体动作与各关系动作分别授权。拥有客户、供应或其他关系权限，只允许读取页面和交易所需的最小主体投影，不自动获得主体编辑权限；名称、身份标识、税号和通用联系资料的修改必须单独具有主体权限。任何关系模块都不得通过保存关系明细顺带修改主体共享资料。

BOB 提供独立主体 current 查询页面，同时保留客户、供应、雇佣、服务和销售合作等关系页面。主体详情只返回当前用户有权读取的关系卡片并允许跳转；无权关系的类型、数量和存在性均不得进入响应。客户与供应页面可以发起 BOB 关系创建；雇佣、服务和销售合作页面的创建、候选和生命周期深链统一进入对应 DCL 页面，并分别校验主体读取或 DCL Party 创建权限、目标关系 DCL 创建权限及经营主体可引用性。共享身份维护和主体合并均进入 DCL Party 页面。
...
服务关系采用 DCL 的连续可用规则：编辑 latest approved 关系时创建 `DRAFT` candidate，旧批准版本继续供新合同、车辆和其他单据引用，候选审核通过后原子切换 BOB current。需要立即阻止新业务时保存 `enabled=false` 的完整 DCL candidate 并批准；已有批准合同及其后续履约、结算和冲销仍使用自身快照完成。

销售合作关系同样采用 DCL 候选换版规则。候选版本移除 `EXTERNAL_PART_TIME` 或 `CHANNEL_PARTNER` 能力前，必须扫描仍以该能力引用此关系的当前客户业务归属；存在引用时拒绝提交和批准，用户必须先通过客户正常编辑、提交和批准流程修改业务归属。历史订单和居间计算快照继续保留原关系与能力事实。
...
主体不使用启停或归档状态。客户与供应关系分别启停；员工、服务和销售合作关系启停只经对应完整 DCL candidate 批准。停用一条关系只阻止该关系形成新交易，不级联停用同一主体的其他关系，也不改变主体身份。
