...
本文中的对象类型、字段和持久化规则只定义目标领域模型，不自行声明当前 HTTP 接口。只有根目录 OpenAPI 已同步定义的实体、路径和数据结构才构成线协议。

以下清单专指当前 OpenAPI 已实现接口。当前实体标识为：
...
结算方式辅助对象及 11 种固定规则由 [AUX 领域](aux.md#33-结算方式)维护。客户选择当前启用的 AUX 结算方式时，客户版本直接保存 `settlementMethodId`、`settlementMethodCode`、`settlementMethodName`、`settlementTermCode`、`settlementRuleType`、`settlementMonthOffset`、`settlementDayOfMonth`、`settlementDayOffset` 和 `settlementSalesSurcharge`，不保存结算方式版本 ID。

上述字段作为一组由后端原子复制和校验的结算快照，客户端不能分别拼装或覆盖其中参数。客户显式重新选择结算方式时才整体替换；AUX 来源后续改名、调价、停用或产生新版本均不追溯改变客户版本。客户提交和审核只校验自身快照完整且参数组合合法，不递归解析来源对象或版本。供应商的结算快照范围由供应商页面用例另行确定。
...
第二类例外只适用于已经拥有有效版本的客户：当 `current_version_id` 指向另一个尚未生效的 `DRAFT` 或 `PENDING` 候选版本时，拥有客户 `delete` 权限的用户可以直接放弃该候选变更。后端在同一事务中锁定客户对象、有效版本和候选版本，校验对象与候选版本 revision、候选版本从未生效且没有外部引用后，删除候选版本的全部审计事件、客户版本明细和版本行，再把 `current_version_id` 恢复为 `effective_version_id` 并递增对象 revision。候选版本即使已经提交、撤回或驳回，其候选审计也随版本物理删除；`next_version_no` 不回退，后续版本不得复用被删除的版本号。删除不改变有效版本、客户编码、启停状态或既有交易引用，任一步失败全部回滚。
...
`categoryId`、`departmentId` 和 `positionId` 分别引用 AUX 的产品分类、部门和岗位；`managerEmployeeId`、`salespersonEmployeeId` 引用 BOB 员工；客户默认经营主体和资金账户归属引用 BOB 经营主体。创建、保存、提交和审核时，这些被引用对象必须存在、处于可引用状态且实体类型匹配。`settlementMethodId` 只记录结算方式辅助对象的选择来源：选择或更换时来源必须当前启用并把关键值复制进引用方版本，此后提交、审核和制单只校验引用方自己的结算快照，不递归校验来源版本。
...
```text
create:  (none)    → DRAFT
delete: DRAFT(v1)  → (none)，或客户候选版本 → 恢复有效版本；仅适用 2.3 节
...
客户允许最后一个 `EFFECTIVE` 版本与一个 `DRAFT` 或 `PENDING` 候选版本并存。修改客户资料时创建候选版本，但不立即冻结或替换最后有效版本；候选版本审核通过时，在同一事务中冻结旧有效版本并切换为新的 `EFFECTIVE` 版本。历史单据继续引用旧版本，新业务从事务提交后引用新版本；候选版本被驳回或撤回不影响最后有效版本。需要立即阻止新销售业务时显式禁用客户，不通过创建候选版本暗示停用。该规则只适用于客户，并优先于本节前述“修改即失去有效版本”的普通 BOB 生命周期。

客户列表始终按稳定对象只返回一行。有候选版本时，列表主名称、编码和其他业务摘要仍取最后有效版本，并额外返回候选版本号及 `DRAFT` 或 `PENDING` 状态，用于展示“有草稿变更”或“待审核”标记；不得把有效版本和候选版本拆成两行，也不得用候选资料冒充当前可交易资料。详情同时返回完整候选版本和只读有效版本，供用户区分正在修改的内容与当前交易事实。

用户放弃候选变更时直接执行 2.3 节的客户候选版本物理删除，不要求先撤回 `PENDING`，也不保留该候选版本及其候选审计。
...
| 物理删除 | `/bob/{entity}/delete`        | 删除满足 2.3 节的首版草稿或客户候选版本 |
...
通用过滤字段为 `keyword`、`status`；类型化过滤字段包括 `customerType`、`operatingEntityId`、`salesAttributionType`、`salesAttributionSubjectId`、`supplierType`、`categoryId`、`departmentId`、`positionId`、`salespersonEmployeeId` 和 `currency`。后端按实体维护白名单并拒绝有值的无关筛选字段：客户支持默认经营主体、主要业务归属类型和主体，供应商支持业务员，员工支持部门和岗位，产品支持产品分类，资金账户支持经营主体和币种。
...
客户存在候选版本时，`query` 的单行投影按 4.2.1 节同时携带有效版本摘要和候选状态；状态筛选分别匹配候选流程状态和是否存在有效版本，不能因为候选为 `DRAFT` 或 `PENDING` 就把仍可交易的客户从有效客户结果中排除。
...
成功响应的 `data` 固定为 `null`。URL 中的 `entity`、对象和版本必须一致，并满足 2.3 节对应例外的全部条件。`objectRevision` 保护稳定对象，`revision` 保护当前版本；任一 revision 过期、状态改变或发现外部引用时均返回现有数据冲突类别，不执行部分删除。首版草稿仍要求没有生命周期历史；客户候选版本则按 2.3 节允许连同候选审计直接删除并恢复有效版本指针。
...
AUX 产品分类、部门、岗位，以及 BOB 经营主体、负责人、主要业务归属和业务员引用在引用方的创建、保存、提交和审核阶段校验。结算方式只在用户选择或更换时校验来源并复制关键值，后续不形成版本依赖。VOU 使用客户默认经营主体、客户主要业务归属或供应商业务员作为新单据默认值时，会在同一事务内解析对应对象的当前有效版本。最终经营主体与资金账户必须引用同一经营主体对象；资金账户有效引用不返回完整账号。
...
21. 产品分类、部门和岗位必须引用实体匹配且当前可用的 AUX 对象；选择结算方式时来源必须当前启用并把关键值完整复制进客户版本，此后来源变化不改变客户结算快照；
...
客户的 `primarySalesAttribution` 和供应商的 `salespersonEmployeeId` 均为必填引用。客户编辑器先选择专职、兼职或经销型，再按类型从员工或其他往来单位最小投影中选择主体；供应商继续只选择员工。前端缺少对应目标实体查询权限或引用列表加载失败时显示明确错误并阻止提交空引用。
