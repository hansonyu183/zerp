...
- 修改已发生业务记录中的历史引用；
- 替交易领域决定需要保存哪些业务快照；
- 在 2.3 节明确允许的客户或供应商候选版本例外之外，物理删除已经进入提交、审核或生效历史的对象、版本和审核记录；
...
默认经营主体、主要业务归属、联系人、业务地址、结算时间、收款方式、运输政策、销售价格政策、信用额度、内部提醒、默认订单备注、应收、开票归属和业绩均属于客户结算子账户。集团不提供默认业务规则，子账户也不继承或覆盖集团字段；任何销售交易只解析一个子账户的完整有效版本。集团不合并子账户余额和信用额度，也不重新引入跨客户、供应商和其他往来单位的通用业务伙伴。

客户结算子账户不维护客户—产品专属配置、默认规格或默认配方对象。定制成品订单继续按 VOU 规则从同一子账户最近一张已检查或已批准销售订单复制配方，订单调整不反写客户主数据。

集团客户使用稳定集团 ID、服务端编码、当前资料 revision 和追加审计，不建立不可变版本，不走提交、审核或候选版本状态机。保存集团资料校验最新 revision 后立即成为之后新业务的当前值；既有发票、收款和收入事实继续使用自身保存的集团名称、税号、地址电话和银行资料快照，不被当前集团修改追溯覆盖。客户结算子账户继续使用本章统一 BOB 版本和审核规则。

集团税号允许在 revision 保护下新增、修改或清空。变更不重分类历史：开票义务只由销售签收批准时保存的集团税号快照决定，历史签收和退货始终沿用原归属。集团当前税号只影响之后批准的销售签收。
...
每个客户结算子账户同一时间只能维护一个 `primarySalesAttribution`。它由互斥的 `type` 和一个主体引用组成：`INTERNAL_EMPLOYEE` 引用当前有效员工，`EXTERNAL_PART_TIME` 与 `DEALER` 引用当前有效其他往来单位。客户版本同时保存主体对象、版本、编码、名称和归属类型快照，不建立独立“业务关系人”对象。专职、兼职的固定按量和溢价收益以及经销型差价均在成交后从实际业务结果分配，不进入客户自动售价成本；自动售价中的第三方居间金额计入该主要业务归属名下，并继续按客户结算子账户分别统计和结算。
...
客户结算子账户版本以 `creditLimits` 保存按交易币种区分的信用额度，同一币种只能一条，金额为非负两位小数；不同币种的额度和占用不得直接相加或通过未定义汇率换算。子账户拥有额度政策，实时已占额度由 ACC 控制事实计算，不在客户版本缓存，集团不另设或汇总信用额度。销售订单超过对应币种额度时仍可保存，但批准必须同时具备普通批准权限、专门的超额批准权限并填写原因；批准结果保存订单额度、当时占用、超额金额、原因和操作者快照。不得沿用 OIT/KY 按业务员汇总客户额度的规则。
...
客户集团和结算子账户都可拥有附件：营业执照、税务及开票资料等共享资料归客户集团，合同、价格、交付约定等只适用于单一交易关系的资料归对应结算子账户。集团附件属于集团当前资料，新增、移除和资料类别变化使用集团 revision 并写追加审计；子账户附件属于具体客户版本，创建候选版本时完整复制关联，只有候选 `DRAFT` 版本可以新增、移除或改类，已生效和历史版本附件只读且不得被候选变化覆盖。每个附件必须选择 AUX“客户资料类别”字典项，并在关联上保存字典对象、版本、编码和名称快照；初始类别为营业执照、税务资料、开票资料、合同、价格约定、交付约定和其他。新增附件或重新分类只能选择当前启用且生效的类别，复制到候选版本的既有附件保留原类别快照，类别后续改名或停用不改写历史。客户没有附件或缺少某一类别不阻止保存、提交、审核或交易。
...
上述字段作为一组由后端原子复制和校验的结算快照，客户端不能分别拼装或覆盖其中参数。客户或供应商显式重新选择结算方式时才整体替换；AUX 来源后续改名、调价、停用或产生新版本均不追溯改变既有版本。提交和审核只校验引用方自身快照完整且参数组合合法，不递归解析来源对象或版本。

客户的 `primarySalesAttribution` 必填并按本章 2.1 节联合引用员工或其他往来单位；创建时必须传入完整的类型、主体对象和版本，保存时省略表示保持，显式 `null` 或空字符串无效。供应商使用 `defaultPurchaserEmployeeId` 引用任意当前有效员工且不附加岗位限制；草稿可以暂缺，提交和审核时必填。该字段只表示我方默认采购员，不表示供应商联系人或供应商业务员。客户和供应商的存量字段迁移规则在契约实现时分别确定，不保留 `salespersonEmployeeId` 兼容字段。
...
经营主体和客户集团的当前非空税号分别在各自实体内大小写不敏感唯一。供应商非空税号、产品条码、车辆 VIN、资金账号在各自实体的当前版本之间大小写不敏感唯一；当前草稿和驳回版本仍占用标识，历史非当前版本不占用。经营主体、客户集团、供应商和其他往来单位之间允许相同税号。车辆车牌沿用相同的当前版本唯一规则。
...
物流平台与普通供应商一样连续生效：编辑期间旧有效版本继续可被车辆和其他新业务引用，候选版本审核通过后原子切换。需要立即阻止平台被新业务引用时必须显式禁用；已有车辆和历史单据引用不被改写。

### 2.5 辅助对象与业务对象引用

`categoryId`、`departmentId` 和 `positionId` 分别引用 AUX 的产品分类、部门和岗位；`managerEmployeeId`、供应商 `defaultPurchaserEmployeeId` 及其他往来单位 `salespersonEmployeeId` 引用 BOB 员工；客户默认经营主体和资金账户归属引用 BOB 经营主体。创建、保存、提交和审核时，这些被引用对象必须存在、处于可引用状态且实体类型匹配。`settlementMethodId` 只记录结算方式辅助对象的选择来源：选择或更换时来源必须当前启用并把关键值复制进引用方版本，此后提交、审核和制单只校验引用方自己的结算快照，不递归校验来源版本。
...
客户和供应商都允许最后一个 `EFFECTIVE` 版本与一个 `DRAFT` 或 `PENDING` 候选版本并存。修改资料时创建候选版本，但不立即冻结或替换最后有效版本；候选版本审核通过时，在同一事务中冻结旧有效版本并切换为新的 `EFFECTIVE` 版本。历史单据继续引用旧版本，新业务从事务提交后引用新版本；候选版本被驳回或撤回不影响最后有效版本。需要立即阻止新销售或采购业务时显式禁用对应对象，不通过创建候选版本暗示停用。该规则只适用于客户和供应商，并优先于本节前述“修改即失去有效版本”的普通 BOB 生命周期。

客户和供应商列表始终按稳定对象只返回一行。有候选版本时，列表主名称、编码和其他业务摘要仍取最后有效版本，并额外返回候选版本号及 `DRAFT` 或 `PENDING` 状态，用于展示“有草稿变更”或“待审核”标记；不得把有效版本和候选版本拆成两行，也不得用候选资料冒充当前可交易资料。详情同时返回完整候选版本和只读有效版本，供用户区分正在修改的内容与当前交易事实。

用户放弃候选变更时直接执行 2.3 节的客户或供应商候选版本物理删除，不要求先撤回 `PENDING`，也不保留该候选版本及其候选审计。

BOB 关联批量转移是受限的跨 BOB 生命周期例外，规则见 4.2.2 节。

#### 4.2.2 BOB 关联批量转移

每个 BOB 类型必须在统一字段元数据中声明全部直接 BOB 关联及其目标实体和字段约束。当前至少包括客户的默认经营主体及三类 `primarySalesAttribution` 主体、供应商的 `defaultPurchaserEmployeeId`、其他往来单位的 `salespersonEmployeeId`、仓库的 `managerEmployeeId`、车辆的 `platformObjectId`、资金账户的经营主体，以及产品配方原料和包装规格引用。新增任何直接引用 BOB 对象的业务字段时必须同步加入该元数据；失效服务遍历元数据，不维护第二份实体白名单。AUX 引用不在本流程范围内。
...
客户或供应商存在候选版本时，`query` 的单行投影按 4.2.1 节同时携带有效版本摘要和候选状态；状态筛选分别匹配候选流程状态和是否存在有效版本，不能因为候选为 `DRAFT` 或 `PENDING` 就把仍可交易的对象从有效结果中排除。

新建客户集团、供应商或其他往来单位时，系统可以按规范化非空税号在另外两类实体中执行精确匹配。跨实体命中只提示可能为同一外部单位，不阻止创建，也不建立持久关联。用户选择复制时，只把其有权读取的来源名称、简称、税号、联系人、电话、邮箱、地址和可识别的开票资料预填到目标集团当前资料或目标对象首版草稿；目标取得自己的编码和生命周期，复制后的任何修改互不传播。来源对象不可读时，不得泄露其存在或资料。客户结算子账户不保存税号，也不参与跨实体税号匹配。
...
`unsubmit` 只允许当前 `PENDING` 版本退回 `DRAFT`。客户或供应商撤回候选提交时，最后有效版本和 `effective_version_id` 均保持不变。除客户和供应商外，`unapprove` 只允许当前 `EFFECTIVE` 版本执行：旧版本冻结为 `INVALID`，业务明细复制到版本号递增的新 `PENDING` 版本。客户和供应商不通过 `unapprove` 开始修改，而是在保存有效资料时按 4.2.1 节创建候选版本。反向动作都要求原因并写入审计。

除客户和供应商外，只有当前版本为 `EFFECTIVE` 时允许启停；客户和供应商只要仍有最后有效版本即可在候选并存期间启停，动作不修改候选版本。成功后对象 revision 增加；重复启用或重复禁用按数据冲突处理。
...
实体、对象和版本必须一致，并满足 2.3 节对应例外的全部条件。任一 revision 过期、状态改变或发现外部引用时均返回现有数据冲突类别，不执行部分删除。首版草稿仍要求没有生命周期历史；客户或供应商候选版本则按 2.3 节允许连同候选审计直接删除并恢复有效版本指针。
...
4. 重新校验关键唯一性和关联约束；
5. 将版本改为 `EFFECTIVE`；客户或供应商候选审核通过时同时把原有效版本冻结为 `INVALID`；
6. 设置 `effective_version_id` 并增加对象 `revision`；
7. 写入审核字段和 `APPROVED` 事件。

### 6.9 审核驳回

只有当前 `PENDING` 版本可驳回，提交人不能审核自己的提交。驳回意见必须满足 OpenAPI 约束；事务内退回 `DRAFT`、清空版本行上的最近提交审核字段并写入 `REJECTED` 事件。客户或供应商候选被驳回时，对象的 `effective_version_id` 继续指向最后有效版本；其他对象保持为空。
...
AUX 产品分类、部门、岗位，以及 BOB 经营主体、负责人、主要业务归属和默认采购员引用在引用方的创建、保存、提交和审核阶段校验。结算方式只在用户选择或更换时校验来源并复制关键值，后续不形成版本依赖。VOU 使用客户默认经营主体、客户主要业务归属或供应商默认采购员作为新单据默认值时，会在同一事务内解析对应对象的当前有效版本。最终经营主体与资金账户必须引用同一经营主体对象；资金账户有效引用不返回完整账号。
...
4. 编辑普通有效对象在同一事务内使旧版本失效并创建新草稿；客户和供应商按 4.2.1 节创建候选版本并保持旧版本有效；
5. 编辑事务失败时旧有效版本保持有效；客户或供应商候选创建失败时不得留下部分候选；
6. 普通对象的新版本待审或被驳回时旧版本不自动恢复；客户和供应商的候选待审、撤回或驳回不影响最后有效版本；
7. 同一对象并发撤销批准只能有一个成功；
...
14. 物流平台编辑期间旧有效版本仍可被相关车辆引用，候选审核通过后原子切换；显式禁用后新业务引用被拒绝；
...
24. 客户集团当前税号在集团间唯一；供应商税号、条码、VIN 和资金账号经过规范化并满足当前版本唯一，历史版本释放后可复用；
