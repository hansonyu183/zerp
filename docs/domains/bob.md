...
本文定义 ZERP **BOB（Business Object）** 领域的业务模型、数据约束和事务边界，覆盖经营主体、仓库、车辆、资金账户、产品与员工的当前业务投影，以及客户、供应商等 BOB 自有业务对象。经营主体、仓库、车辆、资金账户、产品、Party 与员工申报由 DCL 管理；产品类型、人员类别、产品分类、部门、岗位、计量单位、字典、结算方式和收支类型由 AUX 领域管理。HTTP 路径和数据结构以根目录 OpenAPI 为准。
...
- 保存 BOB 自有对象每次新建或编辑产生的独立版本；
- 通过中央 Approval 执行 BOB 自有对象的 `DRAFT`、`PENDING`、`APPROVED` 生命周期；
...
BOB 自有对象列表必须一次返回稳定对象、最新批准版本摘要和开放候选版本摘要；列表补充数据使用固定次数的批量查询，查询次数不得随列表行数增长。DCL 投影对象的 BOB 列表只返回当前正式档案及 `sourceApprovalEntryId`；产品当前单位换算和配方、员工当前雇佣资料均从 current source 指向的 DCL snapshot 读取，并使用固定次数的批量查询。详情读取可以按对象执行实体专属查询。
...
Party stable root 只保存永久 ID 与合并状态；共享身份的候选、版本、审批和审计由 DCL 承载，BOB 仅保存 latest approved 的 current projection。客户、供应、服务和销售合作关系各自使用 BOB 生命周期；雇佣关系的候选、启停和审批由 DCL employee 承载。停用一条关系不得影响同一主体的其他关系。主体作为永久身份保留，业务退出完全由各关系停用表达；误建重复主体只通过主体合并处理。

主体动作与各关系动作分别授权。拥有客户、供应或其他关系权限，只允许读取页面和交易所需的最小主体投影，不自动获得主体编辑权限；名称、身份标识、税号和通用联系资料的修改必须单独具有主体权限。任何关系模块都不得通过保存关系明细顺带修改主体共享资料。

BOB 提供独立主体 current 查询页面，同时保留客户、供应、雇佣、服务和销售合作等关系页面。主体详情只返回当前用户有权读取的关系卡片并允许跳转；无权关系的类型、数量和存在性均不得进入响应。主体详情和各关系列表都可以发起新增关系，但必须复用同一个领域创建能力和事务边界，分别校验主体读取或 DCL Party 创建权限、目标关系创建权限及经营主体可引用性，不维护两套创建规则。共享身份维护和主体合并均进入 DCL Party 页面。
...
主体不使用启停或归档状态。客户、供应、服务和销售合作关系分别启停；员工启停只经 DCL employee candidate 批准。停用一条关系只阻止该关系形成新交易，不级联停用同一主体的其他关系，也不改变主体身份。
