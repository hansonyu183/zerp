...
本文定义 ZERP **BOB（Business Object）** 领域的业务模型、状态机、数据约束、事务边界和前后端职责，作为经营主体、客户、供应商、员工、产品、服务、仓库、车辆和资金账户等业务对象的统一实现规范。产品分类、部门、岗位、计量单位、字典、结算方式和收支类型由 AUX 领域管理。HTTP 路径和数据结构以根目录 OpenAPI 为准。
...
每个 BOB 类型必须在统一字段元数据中声明全部直接 BOB 关联及其目标实体和字段约束。当前至少包括客户的默认经营主体及三类 `primarySalesAttribution` 主体、供应商的 `defaultPurchaserEmployeeId`、其他往来单位的 `salespersonEmployeeId`、仓库的 `managerEmployeeId`、车辆的 `platformObjectId`、资金账户的经营主体，以及产品配方原料和包装规格引用。新增任何直接引用 BOB 对象的业务字段时必须同步加入该元数据；失效服务遍历元数据，不在页面或服务中维护第二份实体白名单。AUX 引用不在本流程范围内。
...
只有当前版本为 `EFFECTIVE` 时允许启停，成功后对象 revision 增加；重复启用或重复禁用按数据冲突处理。
...
客户提交时必须已经保存完整且内部一致的结算快照；缺少关键值时拒绝提交。审核通过前在事务中再次校验同一事实，不允许产生缺少可用结算事实的 `EFFECTIVE` 客户。
...
只有当前 `PENDING` 版本可驳回，提交人不能审核自己的提交。驳回意见必须满足 OpenAPI 约束；事务内退回 `DRAFT`、清空版本行上的最近提交审核字段并写入 `REJECTED` 事件，对象的 `effective_version_id` 保持为空。
...
BOB 页面和 ViewModel。页面支持第 5 节的十四个动作，
并按实体业务规则限制不可用动作。
...
客户的 `primarySalesAttribution` 和供应商生效前的 `defaultPurchaserEmployeeId` 均为必填引用。客户编辑器先选择专职、兼职或经销型，再按类型从员工或其他往来单位最小投影中选择主体；供应商的“默认采购员”只选择员工。前端缺少对应目标实体查询权限或引用列表加载失败时显示明确错误并阻止提交空引用。
