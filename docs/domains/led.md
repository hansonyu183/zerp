...
销售库存由出库单扣减，签收拒收量重新入库；应收只按签收数量计算，拒收和损耗不形成应收。
其它收入即使携带往来方也不改变往来余额。
...
- 单据抬头总额仍为销售总额；
- 数据库列允许为空以兼容存量数据，但新建、保存、审核、批准和最终处理均要求非空；
- 启用重放遇到启用日后缺失采购单价的 `FINALIZED` 居间单据时整体失败并返回待修复单据。

WFL 居间流程使用独立 VOU 原子单据。`goods-receipt` 最终确认时按实收数量和采购价贷记供应商；
`signoff-note` 最终确认时按签收数量和销售价借记客户，并写入空桶增量。反确认追加相反的不可变流水。
来源单据为对应原子 VOU 单据，重开账簿时重放当前有效的已确认收货和签收。任何订阅失败均回滚
WFL、VOU 审计和全部 LED 流水。
