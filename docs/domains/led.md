...
不提供会计科目、复式记账、税额、汇率折算、账龄、逐单核销或库存调拨；库存数量调整仅允许通过已批准的库存盘点单产生。
...
LED 使用 `led_bills` 保存票据稳定身份与固定资料，使用 `led_bill_entries` 保存当前 generation 的位置、方向和用途流水；可用状态不作为可变列保存，而是按同票据、同位置的有效 `IN - OUT` 流水计算。收票批准以 `ASSET/IN` 写入，付票批准以 `PRIMARY/ASSET/OUT` 写入并借记供应商往来；开票批准创建票据主档并以 `PRIMARY/LIABILITY/IN` 写入，同样借记供应商；贴现批准锁定未到期资产票据并以 `PRIMARY/ASSET/OUT` 写入。到期收款批准锁定已到期资产票据并以 `PRIMARY/ASSET/OUT` 写入，到期付款批准锁定已到期负债票据并以 `PRIMARY/LIABILITY/OUT` 写入；两者按真实现金方向形成资金流水且不形成往来流水。付票/贴现入账时逐张锁定可用票据，因此同票据并发操作只有一个事务可成功；开票或贴现的第三方应付利息仅写其他往来贷方流水，不伪造资金流水。反批准仅删除该 generation 的来源流水，若存在下游有效流水则拒绝。票据资料的业务重复键由 `led_bills` 唯一约束保护。

票据台账以带 `customer`、`supplier` 或 `other-party` 类型的来源往来方快照展示和过滤票据，不把开票供应商或其他往来方伪装成客户。票据到期分类和快捷筛选统一按系统业务时区 `Asia/Shanghai` 的自然日计算今天和截止日，不使用数据库会话的 UTC 日期替代业务日期。
...
- 结账日必须是早于当天且不早于活动 LED 切换日的自然月末，并晚于最近结账日；允许一次跨月结账，但从活动 LED 切换月（系统
  零余额切点 `0001-01-01` 则从首张已批准业务单据所在月；已有结账时从最近结账的次月）至目标月的每个月都必须已有一张已批准居间计算单，不能漏月；结账事务
  还会重新生成每张计算单的来源并核对快照，期间来源变化但未重算时拒绝结账。
- 结账前，结账日及以前的全部 VOU 单据必须为 `APPROVED`。
...
并按订单继承的采购单价贷记供应商。反审删除该来源的当前有效流水；订单履约数量变化不改变订单生命周期，也不单独产生流水。
...
- 首次结账、居间计算不漏月的跳月结账、反结最近一期和四类期初快照满足 revision 与原子性；
...
LED（Ledger）把已批准入账的 VOU 业务结果展示为可追溯台账。前端通过 `POST /led/{entity}/{action}` 连接真实后端，不在浏览器计算或持久化权威余额。
...
Playwright 不拦截 LED 请求。LED 流程只连接由 `make e2e` 启动的可销毁隔离后端；
测试仅读取期初、结账历史、流水和余额，不执行结账或反结账，也不能以模拟响应替代。
