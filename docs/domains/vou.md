...
`businessDate` 必须是该月最后一天，币种固定为 `CNY`。结账前，从活动 LED 切换月（已有结账时从最近
结账的次月）至目标结账月的每个自然月都必须存在状态为 `FINALIZED` 的居间计算单；没有符合条件的
...
- 票据成本来源按客户与该客户版本的业务员归集，不按收票单经办人归属；脚本按收票日至票面到期日的
  ...
  当月时没有相同客户和业务员的可计提签收行，该票据保留为未分配来源并顺延；脚本把票据分配到签收行后，
  结果必须保存对应 `billLineIds`，后续月份不再重复提供已分配票据。
