...
订单审批时按往来单位和币种校验 LED 在系统业务时区 `Asia/Shanghai` 当日的可用净余额，未来日期资金不计入。预付要求客户预收或供应商预付覆盖订单总额，并以事务内保留量防止同一笔资金重复审批；反审和实际履约按比例释放。现结要求不存在历史应收/应付，且同一往来单位和币种最多一张仍有未履约数量的现结订单。
...
`0001-01-01` 则从首张已批准业务单据所在月，已有结账时从最近结账的次月）至目标结账月的每个自然月都必须存在状态为 `APPROVED` 的居间计算单；没有符合条件的
...
生成来源并校验哈希；来源变化后必须重新计算。月末结账在同一 LED 控制锁内再次逐月校验已批准计算单的
...
订单始终保持 `APPROVED`，下游数量变化不切换父单生命周期；历史订单可在开放期间创建后续草稿，只有后续单据批准或反批准会受 LED 结账期间约束。
...
- `asset-depreciation` 以 `YYYY-MM` 折旧月份生成只读金额明细，可按资产类别和部门筛选。直线法月折旧额为 `(原值-残值)/使用月数`，末月用差额收尾；购置次月开始折旧，出让或清算当月仍须先批准折旧。同一资产月份不可重复且必须连续。
- `asset-sale` 必须选择客户或其他往来单位；只能选择在用且已折旧至处置月的资产，逐行填写出让金额。批准后资产状态为 `SOLD` 并形成应收，不直接收款。
- `asset-liquidation` 只能选择在用且已折旧至处置月的资产，逐行填写清算原因、残值收入和处置费用。批准后资产状态为 `RETIRED`；首版只记录资产处置历史，不形成资金或往来流水。
- 已有后续折旧或处置历史时，购置或前序折旧不得反批准；处置反批准后资产恢复为在用。首版不提供历史资产期初导入。
...
付票单必须选择供应商，每单引用 1–20 张当前活动账簿中 `AVAILABLE` 的持有票据；每行是 `PRIMARY/ASSET/OUT`，请求只允许提交 `billId`、用途和行备注，服务端在保存和批准时锁定 LED 主档并复制固定资料，客户端不能覆盖票据事实。批准在与 VOU 状态变更相同的事务中对每张票据写入 `ASSET/OUT` 流水，并按票面金额合计借记供应商往来；同一票据并发批准只能有一个成功。反批准删除来源当前 generation 流水并恢复可用；若存在后续票据流水必须拒绝。
