...
订单审批时按往来单位和币种校验 ACC 控制账簿在系统业务时区 `Asia/Shanghai` 当日的可用净余额，未来日期资金不计入。预付要求客户预收或供应商预付覆盖订单总额，并以事务内保留量防止同一笔资金重复审批；反审和实际履约按比例释放。现结要求不存在历史应收/应付，且同一往来单位和币种最多一张仍有未履约数量的现结订单。
