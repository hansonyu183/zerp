...
OIT/KY 的 2026 计算依据仍可作为规则来源，但其“居间费”统称不能直接成为 ZERP 模型。ZERP 的计算结果必须区分本公司专职业务员提成、外部兼职业务员销售服务收益、经销型业务关系差价和真正第三方居间成本。每个客户只有一个主要业务归属；专职与兼职均包含固定按量部分和溢价部分，兼职与经销型关系需要居间服务发票。上述三类业务收益均在成交后分配，不进入自动售价成本。第三方居间包含可同时存在的按 kg 固定部分和差价分成部分，计入该客户主要业务归属名下并按客户区分。各类主体、结果分类及入账边界在客户自动定价用例收敛后确定，未确定前不得把客户返点或具名居间商写入新契约。

现有延期档位仅描述专职或兼职业务关系的固定按量与溢价结果如何随收清延期扣减，不决定经销型差价和第三方居间的算法。固定金额、溢价阶梯、票据成本分摊以及 `fd_price` 差价分成的精确规则继续由版本化脚本保存；历史单据始终保留当次计算稿。
...
脚本结果可以随规则变化编辑；历史单据始终保留当次计算稿。结果分类不得再使用 `REBATE` 表达客户返点，也不得以一个 `INTERMEDIARY` 混装专职、兼职、经销型和第三方居间。精确分类及哪些结果形成 ACC 其他往来应付，在业务归属主体与结算身份确认后补齐。
...
往来收付款固定为六个独立实体：`sales-receipt`（销售收款）、`sales-refund`（销售退款）、`purchase-payment`（采购付款）、`purchase-refund`（采购退款）、`other-receipt`（其他往来收款）、`other-payment`（其他往来付款）。销售退款固定客户结算子账户，采购实体固定供应商并结算贸易往来；其他往来收付款必须从客户、供应商、其他单位、员工中选择主体类型并始终结算其他往来。原 `REBATE` 客户返点分类不再使用；专职、兼职、经销型及第三方居间的分类随本章 2.4 节业务身份边界一并确定。各实体拥有独立路由与权限，单据币种必须与资金账户币种一致。

销售收款保留一张真实银行来款单，保存集团客户、经营主体、该主体名下资金账户、必填经办人、业务日期、币种、总金额和备注，并以一至多条分摊行把总金额分配到该集团下、属于同一经营主体的客户结算子账户未结应收。系统先选已逾期应收，再按到期日、单据日期、单号升序稳定分配，仍有来款余额时继续按相同顺序处理未到期应收；金额相同或并发读取不得改变建议顺序。不同经营主体的应收不能进入同一张销售收款分摊。

分摊合计必须精确等于来款总额才可批准。集团只有一个子账户时系统直接生成分摊且无需人工确认；存在多个子账户时展示建议的逐子账户和逐应收明细，用户确认或调整后才能批准。批准按最终分摊分别贷记各子账户贸易往来，不在集团层形成共享应收或收款余额，也不把一笔银行流水拆成多张虚假收款单。其它五类收付款仍按各自单一往来方处理；其首版不关联或核销来源单据。

来款超过该集团在本张收款经营主体下的全部可核销应收时，剩余金额仍必须选择该集团下的具体客户结算子账户，并在同一经营主体边界内作为该子账户预收款。单子账户集团自动使用唯一子账户；多子账户集团必须由用户明确分配，可以拆分到多个子账户。集团层不得保存未分配来款、预收或共享余额。

开票义务不使用客户“是否开票”字段。销售签收批准确认收入和应收时同时保存客户集团名称、税号及税务资料快照和订单经营主体法定资料快照：客户税号非空则按客户结算子账户和经营主体形成待开票金额，并只能由该经营主体开票；客户税号为空则按同一子账户和经营主体进入未开票收入申报。销售退货按原签收保存的税号状态、子账户和经营主体冲减，不读取退货时集团、客户或经营主体当前资料。后续主数据变化不重分类历史。

未开票收入不另建手工申报 VOU。系统按自然月、经营主体和客户结算子账户汇总已批准销售签收减已批准销售退货，同时保留双方税务快照、来源签收、退货、产品、数量、金额和税额等逐单明细供 RPT 查询、导出和申报核对。集团层只用于筛选和汇总，不替代子账户明细。每张发票仍唯一归属一个客户结算子账户和一个经营主体并保存当时双方税务资料快照，不得跨子账户或经营主体开票。
...
- `asset-sale` 必须选择客户或其他往来单位；只能选择 ACC 登记的在用资产，逐行填写出让金额。批准后资产状态为 `SOLD` 并形成应收，不直接收款。
...
开票单必须选择供应商，每单可新建 1–20 张公司自开票据；服务端生成 `billId`，并强制每行 `PRIMARY/LIABILITY/IN`。可录入至多 20 条实际保证金、手续费等资金行，按提交的 `IN/OUT` 原样形成多资金账户流水。计息方式仅为 `BANK_DEDUCTED` 或 `THIRD_PARTY_PAYABLE`；后者必须选择有效其他往来单位，批准时按后端整数公式计算的票据利息合计贷记该其他往来，不伪造资金支出；银行扣息不形成其他往来。批准按票面金额合计借记供应商往来；反批准与所有票据单据一样删除来源流水，若有后续票据流水或结账冻结则拒绝。

贴现单必须选择其他往来单位作为贴现方；每单引用 1–20 张当前 `AVAILABLE`、未支付、未贴现且未到期的资产票据，行固定为 `PRIMARY/ASSET/OUT`，服务端锁定并复制票据资料。行年利率是本次贴现率，利息天数为到期日减业务日，利息由后端整数公式重算，不沿用票据主档利息。可录入至多 20 条真实现金行：净到账为资金流入，实际另付手续费为资金流出，贴现净到账严格为资金流入减资金流出且必须大于零，不得加入票面金额；不得伪造银行直接扣息资金流水。计息方式为 `BANK_DEDUCTED` 或 `THIRD_PARTY_PAYABLE`，后者选择其他往来计息方并贷记利息应付；`withRecourse` 仅作快照保存与查询。批准/反批准与票据台账、下游阻断和结账冻结保持同一事务语义。
...
- 单据状态固定为 `DRAFT ⇄ CHECKED ⇄ APPROVED`。`check/uncheck`、`approve/unapprove` 是用户动作；页面将 `APPROVED` 展示为“已批准”。
...
- AUX 结算方式不是 VOU 的运行时引用；VOU 直接消费客户版本保存的结算快照，不提交或解析结算方式版本 ID。
- 已有销售草稿的主要业务归属或采购草稿的采购员没有被用户改动时，保存请求省略对应字段，以保留后端快照；新建时由后端分别使用客户主要业务归属或供应商默认采购员。
...
Playwright 不拦截 VOU 或 BOB 请求，只连接仓库根目录 `make e2e` 创建的可销毁隔离后端。命令负责迁移和播种数据库，并为每个 Playwright worker 通过真实 API 创建独立操作员、复核员、客户、供应商、员工、仓库、产品、物流平台、车辆、资金账户、库存和会话；结束后自动清理数据库、附件和原生进程。测试不读取 `E2E_VOU_*` 变量，也不接受共享外部测试资料。
