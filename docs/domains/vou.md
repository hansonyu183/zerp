...
票据共用 `bill-receipt`、`bill-payment`、`bill-issue`、`bill-discount`、`bill-maturity` 五类原子单据的固定资料模型；当前开放 `bill-receipt` 与 `bill-payment`。收票单固定客户往来方与经办人快照，单头保存内部客户票据成本率、到期/计息/追索配置及可选计息方快照。每单最多 20 张票据和 20 条资金行。
