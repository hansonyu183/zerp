...
十六类单据均提供查询、查看、保存、删除草稿、生命周期、审计和附件动作；实际可用性继续受单据
状态、上下级关系和精确权限约束。`sale-outbound`、`sale-delivery`、`sale-signoff` 不提供
公开 `create` 权限，由 WFL 事件订阅自动创建；其余十三类单据允许按各自规则创建。
...
VOU 前端对接后端 `POST /vou/{entity}/{action}` 契约，提供十六类独立单据的查询、查看、
审核、批准、执行、反向流转、附件和审计界面；其中十三类提供人工制单入口。后端领域文档和
