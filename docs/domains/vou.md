...
订单审批时按往来单位和币种校验 LED 在系统业务时区 `Asia/Shanghai` 当日的可用净余额，未来日期资金不计入。预付要求客户预收或供应商预付覆盖订单总额，并以事务内保留量防止同一笔资金重复审批；反审、短结和实际履约按比例释放。现结要求不存在历史应收/应付，且同一往来单位和币种最多一张未完成现结订单。
...
检查器只查询会写入 LED、`status=APPROVED` 且尚无任何 LED 来源流水的 VOU，输出总数、按实体汇总以及每张单据的实体、单号、业务日期和状态；本来不写入 LED 的实体和已经存在 LED 来源流水的单据不属于遗留。检查结果不输出正文、附件或凭证。无遗留时退出 `0`；发现遗留或查询失败时明确非 `0`。检查器绝不修改 VOU、LED 或审计记录，也不会自动修复。
...
已进入结账期间的完成订单不能重新打开，因此不得再创建会触发重新打开的入库、退货或其他后续单据。
