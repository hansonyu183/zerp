...
在已授权的环境停止旧版本写入后，从受控环境注入 `TARGET_DATABASE_URL` 与匹配的 `TARGET_DATABASE_SCOPE`，运行 `pnpm --filter @zerp/api migrate:bob-product`，成功后运行 `pnpm --filter @zerp/api sync:catalog`。不得执行会隐式清库的 Make 聚合命令。
