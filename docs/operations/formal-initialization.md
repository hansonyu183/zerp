...
6. 独立回读全部表的行数。允许非空的初始化表为 `app_permissions`、`app_users`、`app_roles`、`app_user_roles`、`app_system_parameters`、`app_role_code_counters`、`archive_code_counters`、`rpt_code_counter`、`acc_mapping_vou_entities`，以及 `acc_books`（1 本内账）、`acc_subjects`（185 个科目）和 `acc_book_access`（2 位管理员）；登录验收后还允许本次 `app_sessions` 和 `app_audit_events`。其他业务表必须为空；管理员只有明确配置的两人，`superadmin` 无逐项权限关联。
