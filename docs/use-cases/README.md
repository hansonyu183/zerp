...
本目录按用户可见页面记录前端编排、后端协作次序、异常分支和验收场景。业务规则仍以 [`docs/domains/`](../domains/) 为唯一事实来源。#366 前，live Go HTTP 路径和数据结构以 [`contracts/openapi/`](../../contracts/openapi/) 为唯一事实来源；隔离 target 路径和结构只从 `apps/api/` 的可执行 Hono/Zod 路由生成，不与 live 契约组合。描述 target 编排的用例必须明确标出该边界；尚未进入当前 target 的 #366 future 流程也必须显式标记。
...
- 按所属运行线引用 live OpenAPI 或 target 可执行路由，不维护请求或响应结构副本；
