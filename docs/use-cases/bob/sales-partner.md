...
- 领域规则以 `docs/domains/bob.md` 为准；HTTP 契约以 `contracts/openapi/openapi.yaml` 的 `sales-partner` 操作为准。
...
- 能力仅允许 `EXTERNAL_PART_TIME`（外部兼职销售）和 `CHANNEL_PARTNER`（渠道商）；草稿可以为空，提交和审核时至少选择一种。编辑有效版本时，后端创建候选草稿版本。
