...
1. 页面入口为 `/dcl/employee`，是员工新建、编辑、启停、提交、撤回、驳回、批准、反批准、删除、版本与审计的唯一维护入口；工作台、审批待办和审批记录深链均进入本页。
...
3. 批准和反批准后的 BOB latest-approved 可见性、历史精确引用 blocker 与历史资料规则由 [DCL 员工申报](../../domains/dcl.md#36-员工申报) 统一定义。页面只展示结果，不自行迁移、清空或重建引用。
...
3. 历史 VOU/ACC 在员工后续改版、启停、反批准或 Party 更新后仍保留原 stable ID、Approval Entry 与业务快照。
