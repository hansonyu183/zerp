...
- 路由：`/vou/bill-maturity`，公共 Draft、查询、详情与生命周期编排见 [VOU 页面公共用例](README.md)。
- 业务不变量以 [VOU 领域文档](../../domains/vou.md#313-票据管理) 为准；请求与响应只采用可执行 Hono 路由。

## `VOU-BILL-MATURITY-01` 业务工作区

1. 选择收票或付票到期类型，维护到期票据与资金结算明细。
2. 用户可新建本地 Draft，或从服务器详情建立变更 Draft；页面使用票据业务家族编辑器，字段变化保存到当前浏览器。
3. 新增与变更分别调用 `/vou/bill-maturity/submit-new`、`/vou/bill-maturity/submit-change`；失败保留 Draft，成功只删除本次 Draft。
