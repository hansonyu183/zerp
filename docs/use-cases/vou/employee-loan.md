...
- 路由：`/vou/employee-loan`，公共 Draft、查询、详情与生命周期编排见 [VOU 页面公共用例](README.md)。
- 业务不变量以 [VOU 领域文档](../../domains/vou.md#37-员工借款还款与借款核销) 为准；请求与响应只采用可执行 Hono 路由。

## `VOU-EMPLOYEE-LOAN-01` 业务工作区

1. 选择员工、资金账户和经办人，登记借款金额。
2. 用户可新建本地 Draft，或从服务器详情建立变更 Draft；页面使用资金业务家族编辑器，字段变化保存到当前浏览器。
3. 新增与变更分别调用 `/vou/employee-loan/submit-new`、`/vou/employee-loan/submit-change`；失败保留 Draft，成功只删除本次 Draft。
