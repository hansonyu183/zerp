...
- 路由：`/vou/sales-refund`，公共 Draft、查询、详情与生命周期编排见 [VOU 页面公共用例](README.md)。
- 业务不变量以 [VOU 领域文档](../../domains/vou.md#36-往来收款与往来付款) 为准；请求与响应只采用可执行 Hono 路由。

## `VOU-SALES-REFUND-01` 业务工作区

1. 选择客户、资金账户与经办人，登记销售退款金额。
2. 用户可新建本地 Draft，或从服务器详情建立变更 Draft；页面使用资金业务家族编辑器，字段变化保存到当前浏览器。
3. 新增与变更分别调用 `/vou/sales-refund/submit-new`、`/vou/sales-refund/submit-change`；失败保留 Draft，成功只删除本次 Draft。
