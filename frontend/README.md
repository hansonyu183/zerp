...
5. 删除旧代码、专属测试和失效导出，为新模块补充测试并迁移覆盖率要求。
...
Vitest 和组件测试可使用 MSW，handlers 只能存在于测试目录。重点覆盖 API 错误归一化、ViewModel 状态、会话恢复、权限菜单、表单与并发保护。覆盖率要求采用全局基线加关键模块独立阈值，新增职责不得靠其他高覆盖文件抵消回退。
