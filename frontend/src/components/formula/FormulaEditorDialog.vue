...
const { options, loading, errorMessage, search: searchMaterials } =
...
watch(
  ...
  (open) => {
    if (!open) return
    draft.value = props.modelValue ? structuredClone(props.modelValue) : emptyFormula()
...
function defaultUnit(product: FormulaMaterialReference): FormulaUnitSnapshot | null {
  const conversions = product.unitConversions ?? []
  return structuredClone(
    conversions.find((item) => item.unit.objectId === product.defaultInputUnitId)?.unit ??
      conversions[0]?.unit ??
      null,
  )
}

function emptyQuantity(product?: FormulaMaterialReference): FormulaQuantitySnapshotDraft {
  return { enteredQuantity: '', enteredUnit: product ? defaultUnit(product) : null, baseQuantity: '' }
}
...
function addComponent(): void {
  ...
  draft.value.components.push({ key: crypto.randomUUID(), material: null, quantity: emptyQuantity() })
}
...
function validQuantity(value: FormulaQuantitySnapshotDraft): boolean {
  return Boolean(
    value.enteredUnit && isQuantity(value.enteredQuantity) && isQuantity(value.baseQuantity),
  )
}
...
function submit(): void {
  ...
  if (!validQuantity(draft.value.output)) {
    validationMessage.value = '请完整填写配方产量的录入数量、单位和 Base Quantity。'
    return
  }
  if (
  ...
  const ids = draft.value.components.map((component) => component.material!.objectId)
...
<template>
  <v-dialog :model-value="open" max-width="1040" persistent @update:model-value="emit('update:open', $event)">
    <v-card rounded="xl">
      ...
      <v-card-text>
        ...
        <div class="formula-dialog__table-wrap responsive-table-wrap">
          <v-table class="formula-dialog__table responsive-table responsive-table--form">
            <thead><tr><th>#</th><th>原材料</th><th>录入数量</th><th>录入单位</th><th>Base Quantity</th><th v-if="editable" /></tr></thead>
            <tbody>
              <tr v-for="(component, index) in draft.components" :key="component.key">
                ...
                <td data-label="原材料">
                  <v-autocomplete
                    ...
                    :item-title="materialTitle" :items="options" :loading="loading"
                    no-filter return-object variant="underlined"
                    @update:model-value="changeMaterial(component, $event)"
                    @update:search="searchMaterials($event ?? '')"
                  />
                  <span v-else>{{ component.material ? materialTitle(component.material) : '—' }}</span>
                </td>
                <td data-label="录入数量">
                  <CompactTableField
                    v-model="component.quantity.enteredQuantity"
                    :disabled="!editable" inputmode="decimal"
                    @update:model-value="component.material && suggest(component.quantity, component.material)"
                  />
                </td>
                <td data-label="录入单位">
                  <v-select
                    v-model="component.quantity.enteredUnit"
                    :disabled="!editable" density="compact" hide-details
                    :item-title="unitTitle"
                    :items="component.material?.unitConversions?.map((item) => item.unit) ?? []"
                    return-object variant="underlined"
                    @update:model-value="component.material && suggest(component.quantity, component.material)"
                  />
                </td>
                <td data-label="Base Quantity">
                  <CompactTableField v-model="component.quantity.baseQuantity" :disabled="!editable" inputmode="decimal" />
                </td>
                <td v-if="editable" class="responsive-table__actions" data-label="操作">
                  <v-btn :aria-label="`删除第 ${index + 1} 行原料`" color="error" icon="mdi-delete-outline" variant="text" @click="removeComponent(index)" />
                </td>
              </tr>
              <tr v-if="draft.components.length === 0" class="responsive-table__empty-row">
                <td :colspan="editable ? 6 : 5" class="text-center py-8">暂无原料</td>
              </tr>
            </tbody>
          </v-table>
        </div>
        <v-btn v-if="editable" class="mt-3" :disabled="draft.components.length >= 200" prepend-icon="mdi-plus" variant="tonal" @click="addComponent">添加原料</v-btn>
      </v-card-text>
      <v-card-actions class="px-6 pb-5">
        <v-spacer />
        <v-btn variant="text" @click="emit('update:open', false)">{{ editable ? '取消' : '关闭' }}</v-btn>
...
<style scoped>
.formula-dialog__base { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; margin-bottom: 16px; }
.formula-dialog__table-wrap { overflow-x: auto; }
.formula-dialog__table { min-width: 900px; }
.formula-dialog__table :deep(.v-input) { min-width: 140px; }
@media (max-width: 720px) { .formula-dialog__base { grid-template-columns: 1fr; } }
</style>
