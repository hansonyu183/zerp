...
const templateView = ref<'DEFAULT' | 'BUSINESS'>('BUSINESS')
const vm = reactive(
  createMenuViewModel({
    load: getMenu,
...
<template>
  <v-container fluid class="pa-5 pa-md-8">
    ...
    <div class="d-flex flex-wrap align-center ga-3 mb-6">
      ...
      <v-chip v-if="vm.data" variant="tonal">
        当前：{{ vm.data.mode === 'DEFAULT' ? '系统默认' : '业务归类菜单' }}
      </v-chip>
      <v-chip v-if="vm.data" variant="tonal">
        当前版本：{{ vm.data.revision }}
      </v-chip>
    </div>
    ...
    <template v-if="vm.data">
      <v-tabs v-model="templateView" class="mb-4" color="primary">
        <v-tab value="BUSINESS">业务归类菜单</v-tab>
      ...
      <section v-if="templateView === 'DEFAULT'" aria-label="系统默认菜单">
        <v-alert class="mb-4" type="info" variant="outlined">
          系统默认菜单为只读。切换到“业务归类菜单”后可编辑自定义归类。
        </v-alert>
        <v-card
      ...
      <section v-else aria-label="业务归类模板">
        <v-alert v-if="vm.dirty" class="mb-4" color="warning" variant="tonal">
          有未保存修改。保存后立即成为当前业务菜单。
        </v-alert>
  ...
  <v-dialog v-model="vm.resetConfirmationOpen" max-width="520">
    <v-card rounded="xl" title="恢复业务归类模板">
      <v-card-text>
        此操作会替换当前业务菜单并立即生效，现有分组、名称、图标、排序和启停设置都会丢失。确认继续吗？
      </v-card-text>
  ...
  <v-dialog v-model="vm.activationConfirmationOpen" max-width="520">
    <v-card rounded="xl" title="应用菜单方式">
      <v-card-text> 将立即切换当前会话主导航。确认继续吗？ </v-card-text>
      <v-card-actions class="px-6 pb-5">
        <v-spacer />
