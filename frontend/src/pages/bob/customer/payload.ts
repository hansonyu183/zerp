...
export function customerAccountPayload(
  ...
  if (!account.operatingEntity || !subject)
    throw new Error('客户账户的经营主体和业务归属主体不能为空。')
  return {
    name: account.name.trim(),
    customerTypeCode: account.customerTypeCode,
    shortName: account.shortName.trim() || null,
    contactName: account.contactName.trim() || null,
    contactPhone: account.contactPhone.trim() || null,
    email: account.email.trim() || null,
    address: account.address.trim() || null,
    operatingEntityId: account.operatingEntity.objectId,
...
export function customerCreatePayload(
  ...
  if (form.party.mode === 'EXISTING')
    return { partyId: form.party.partyId, data }
  return {
    newParty: {
      kind: form.party.kind,
      legalName: form.party.legalName.trim(),
      ...
      strongIdentifiers: form.party.identifierValue.trim()
        ? [
            {
              type: form.party.identifierType,
              value: form.party.identifierValue.trim(),
            },
          ]
        : [],
    },
