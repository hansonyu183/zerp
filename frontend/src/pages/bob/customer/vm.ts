...
type CustomerDetailWire = components['schemas']['CustomerDetailView']

function toReference(
  value:
    { sourceObjectId: string; code: string; name: string } | null | undefined,
  entity: CustomerReference['entity'],
...
function accountForm(
  ...
  return {
    ...
    operatingEntity: toReference(data.operatingEntity, 'operating-entity'),
    settlementMethod: toReference(data.settlementMethod, 'settlement-method'),
    paymentMethod: toReference(data.paymentMethod, 'payment-method'),
    ...
    primarySalesAttribution: {
      ...
      subject: attribution.subjectObjectId
        ? {
            objectId: attribution.subjectObjectId,
            approvalEntryId: attribution.subjectApprovalEntryId,
            code: attribution.subjectCode,
            name: attribution.subjectName,
            entity: salesAttributionSubjectEntity(attribution.type),
          }
        : null,
    },
...
function detailFromWire(raw: CustomerDetailWire): CustomerDetail {
  return {
    ...
    accounts: raw.accounts.map((entry) => {
      const version = entry.openVersion ?? entry.latestApproved
      if (
        !version ||
        !entry.objectId ||
        !entry.code ||
        entry.objectRevision === undefined
      )
        throw new Error('客户关系包含不可读取的结算账户。')
      return {
        ...
        data: accountForm(version.data, entry.code),
...
function listItem(
  item: components['schemas']['CustomerListItem'],
): CustomerListItem {
  const current = item.openVersion ?? item.latestApproved
  return {
    ...
    customerType: current?.customerTypeCode ?? '',
    hasCandidate: item.openVersion !== null,
...
export function useCustomerViewModel() {
  ...
  const referenceOptions = ref<
    Record<CustomerReferenceKey, CustomerReference[]>
  >({
    operatingEntity: [],
    settlementMethod: [],
    paymentMethod: [],
  ...
  const canAddAccount = computed(
    () =>
      ...
      session.can('/bob/customer-account/create') &&
      requiredReferences.every((path) => session.can(path)),
  )
  const canDeleteAccount = computed(() =>
    session.can('/bob/customer-account/delete'),
  )
  ...
  const selectedAccount = computed(
    () =>
      detail.value?.accounts.find(
        (account) => account.objectId === selectedAccountId.value,
      ) ?? null,
  )
  const formErrors = computed(() => [
    ...(mode.value === 'create' &&
    form.value.party.mode === 'EXISTING' &&
    !form.value.party.partyId
      ? ['请选择已有主体。']
      : []),
    ...(mode.value === 'create' &&
    form.value.party.mode === 'NEW' &&
    !form.value.party.legalName.trim()
      ? ['请输入主体法定名称。']
      : []),
    ...(form.value.account.name.trim() ? [] : ['请输入客户账户名称。']),
    ...(form.value.account.operatingEntity ? [] : ['请选择经营主体。']),
  ...
  const referenceConfigs: Record<
    CustomerReferenceKey,
    CustomerReference['entity']
  > = {
  ...
  function queryFilters() {
    const all = { keyword: keyword.value.trim(), ...filters.value }
    ...
    try {
      const result =
        ...
        entity === 'sales-partner'
          ? await customerApi.queryBobReferences({
              entity: entity as ApiPostRequest<'bob/reference/query'>['entity'],
      ...
      const selected = [
        ...
        form.value.account.primarySalesAttribution.subject,
      ].filter((item): item is CustomerReference => item?.entity === entity)
  ...
  function changeSalesAttributionType(value: unknown) {
    if (
    ...
    void loadReferenceOptions(
      salesAttributionSubjectEntity(value) === 'employee'
        ? 'employee'
        : 'salesPartner',
    )
  }
  ...
  function openAddAccount() {
    if (!canAddAccount.value || !detail.value) return
    mode.value = 'add-account'
    form.value = createCustomerForm()
    form.value.party.mode = 'EXISTING'
    form.value.party.partyId = detail.value.partyId
    form.value.account.operatingEntity = {
      objectId: detail.value.operatingEntityId,
      approvalEntryId: '',
      code: detail.value.operatingEntityCode,
      name: detail.value.operatingEntityName,
      entity: 'operating-entity',
    }
    savedFormSignature = JSON.stringify(form.value)
    workspaceOpen.value = true
  ...
  async function openEdit(row: CustomerListItem) {
    if (!session.can('/bob/customer/get')) return
    loading.value = true
    errorMessage.value = null
    try {
      const result = await customerApi.get({ objectId: row.objectId })
      if (!result.data) throw new Error('客户关系不存在。')
      detail.value = detailFromWire(result.data)
      const account =
        detail.value.accounts.find((item) => item.objectId === row.objectId) ??
        detail.value.accounts[0]
      if (!account) throw new Error('客户关系没有结算账户。')
      selectedAccountId.value = account.objectId
      form.value = {
        party: createCustomerForm().party,
        account: structuredClone(account.data),
      }
      mode.value = 'edit'
      savedFormSignature = JSON.stringify(form.value)
      workspaceOpen.value = true
  ...
  function selectAccount(accountId: string) {
    const account = detail.value?.accounts.find(
  ...
  async function save(): Promise<boolean> {
    ...
    try {
      if (mode.value === 'create')
        await customerApi.create(customerCreatePayload(form.value))
      else if (mode.value === 'add-account' && detail.value)
        await customerApi.accountAdd({
          customerRelationshipId: detail.value.objectId,
          data: customerAccountPayload(form.value.account),
        })
      else if (mode.value === 'edit' && selectedAccount.value)
        await customerApi.save({
          objectId: selectedAccount.value.objectId,
          approvalEntryId: selectedAccount.value.approvalEntryId,
          approvalRevision: selectedAccount.value.approvalRevision,
          data: customerAccountPayload(form.value.account),
        })
      workspaceOpen.value = false
      savedFormSignature = JSON.stringify(form.value)
  ...
  async function removeAccount(account: CustomerAccount) {
    if (
      ...
      detail.value.accounts.length <= 1 ||
      account.status !== 'DRAFT'
    )
      return false
    try {
      await customerApi.accountDelete({
        objectId: account.objectId,
        approvalEntryId: account.approvalEntryId,
        approvalRevision: account.approvalRevision,
        objectRevision: account.objectRevision,
      })
      await openRelationship(detail.value.objectId)
      successMessage.value = '结算账户已删除。'
      return true
    } catch (error) {
      errorMessage.value = getErrorMessage(error)
      return false
  ...
  async function openRelationship(objectId: string) {
    const result = await customerApi.get({ objectId })
    if (!result.data) throw new Error('客户关系不存在。')
    detail.value = detailFromWire(result.data)
    if (
      !detail.value.accounts.some(
        (item) => item.objectId === selectedAccountId.value,
      )
    )
      selectedAccountId.value = detail.value.accounts[0]?.objectId ?? ''
  }
  function canLifecycleFor(
    row: CustomerListItem,
    action: CustomerLifecycleAction,
  ) {
    if (!session.can(`/bob/customer-account/${action}`)) return false
    ...
    if (action === 'unapprove')
      return row.status === 'APPROVED' && !row.hasCandidate
    if (action === 'enable')
      return row.status === 'APPROVED' && !row.hasCandidate && !row.enabled
    if (action === 'disable')
      return row.status === 'APPROVED' && !row.hasCandidate && row.enabled
    return false
  }
  async function runLifecycle(
    ...
    try {
      ...
      if (action === 'enable' || action === 'disable')
        await customerApi[action]({
          objectId: row.objectId,
          objectRevision: row.objectRevision,
        })
      else if (action === 'unsubmit') await customerApi.unsubmit(base)
      else if (action === 'reject')
        await customerApi.reject({ ...base, reason: normalized })
      else if (action === 'unapprove')
        await customerApi.unapprove({ ...base, reason: normalized })
      else if (action === 'submit') await customerApi.submit(base)
      else await customerApi.approve(base)
      await query()
      successMessage.value = `${row.code} 已完成生命周期操作。`
      return true
    } catch (error) {
      errorMessage.value = getErrorMessage(error)
      return false
  ...
  async function refreshAttachments() {
    if (detail.value) await openRelationship(detail.value.objectId)
  }
  async function uploadAttachments(
    ...
    const owner =
      scope === 'RELATIONSHIP' ? detail.value : selectedAccount.value
    if (
      !owner ||
  ...
  async function downloadAttachment(
    ...
    const owner =
      scope === 'RELATIONSHIP' ? detail.value : selectedAccount.value
    if (!owner || !canAttachmentDownload.value || attachmentLoading.value)
      return
    attachmentLoading.value = true
    try {
  ...
  async function removeAttachment(
    ...
    const owner =
      scope === 'RELATIONSHIP' ? detail.value : selectedAccount.value
    if (!owner || !canAttachmentRemove.value || attachmentLoading.value) return
    attachmentLoading.value = true
    try {
      await customerApi.attachmentRemove({
        scope,
        ownerId: owner.objectId,
        revision: owner.objectRevision,
        fileId: attachment.fileId,
