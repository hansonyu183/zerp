...
const columns: readonly BusinessObjectColumn<(typeof vm.rows)[number]>[] = [
  { key: 'code', label: '编码', value: (row) => row.code, sizing: 'compact' },
  {
    ...
    label: '主体',
    value: (row) =>
      String((row.openVersion ?? row.latestApproved)?.summary.name ?? ''),
    sizing: 'fluid',
  },
  {
    key: 'department',
    ...
    value: (row) =>
      String(
        (row.openVersion ?? row.latestApproved)?.summary.departmentName ?? '—',
      ),
  },
  {
    key: 'position',
    ...
    value: (row) =>
      String(
        (row.openVersion ?? row.latestApproved)?.summary.positionName ?? '—',
      ),
  },
  {
    key: 'status',
...
<template>
  <v-container fluid class="employee-page pa-5 pa-md-8">
    ...
    >
      <template #filters>
        <v-autocomplete
          v-model="vm.departmentId"
          :items="vm.departmentOptions"
          :item-title="vm.title"
          item-value="objectId"
          clearable
          label="部门"
          variant="outlined"
          @update:search="vm.loadReferences('department', $event ?? '')"
        />
        <v-autocomplete
          v-model="vm.positionId"
          ...
          label="岗位"
          variant="outlined"
          @update:search="vm.loadReferences('position', $event ?? '')"
        />
      </template>
    </BusinessObjectList>
  </v-container>
    ...
    <v-form class="employee-page__form" @submit.prevent="vm.save">
      <header class="employee-page__header">
        ...
        <div class="d-flex ga-2">
          <v-btn variant="text" @click="vm.drawerOpen = false">取消</v-btn
      ...
      <section>
        ...
        />
        <div v-else class="employee-page__grid">
          <v-select
      ...
      <section>
        ...
        <div class="employee-page__grid">
          <v-autocomplete
            ...
            label="经营主体"
            variant="outlined"
            @update:search="vm.loadReferences('operating', $event ?? '')"
          /><v-autocomplete
            v-model="vm.form.departmentId"
            :items="vm.departmentOptions"
            :item-title="vm.title"
            item-value="objectId"
            clearable
