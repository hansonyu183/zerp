  ...
  <v-row dense>
    ...
    <v-col cols="12">
      <div class="d-flex align-center">
        ...
        <v-spacer />
        <v-btn v-if="!readonly" size="small" @click="vm.addAccount(form)"
          >新增账户</v-btn
        >
      </div>
    </v-col>
      ...
      <v-card variant="outlined">
        <v-card-title class="d-flex align-center text-subtitle-1">
          ...
          <v-radio-group
            ...
            hide-details
            inline
            @update:model-value="vm.setDefaultAccount(index, form)"
          >
            <v-radio :value="true" label="默认" />
          </v-radio-group>
          <v-btn
            v-if="!readonly && form.accounts.length > 1"
            icon="mdi-delete-outline"
            variant="text"
            @click="vm.removeAccount(index, form)"
          />
        </v-card-title>
        <v-card-text>
          <CustomerAccountFields
            ...
            :readonly="readonly"
            :reference-error="{
              customerTypeId: vm.referenceErrorForAccount(
                index,
                'customerTypeId',
              ),
              settlementMethodId: vm.referenceErrorForAccount(
                index,
                'settlementMethodId',
              ),
              paymentMethodId: vm.referenceErrorForAccount(
                index,
                'paymentMethodId',
              ),
              primarySalesAttributionSubjectObjectId:
                vm.referenceErrorForAccount(
                  index,
                  'primarySalesAttributionSubjectObjectId',
                ),
            }"
            :reference-loading="{
              customerTypeId: vm.referenceLoadingForAccount(
                index,
                'customerTypeId',
              ),
              settlementMethodId: vm.referenceLoadingForAccount(
                index,
                'settlementMethodId',
              ),
              paymentMethodId: vm.referenceLoadingForAccount(
                index,
                'paymentMethodId',
              ),
              primarySalesAttributionSubjectObjectId:
                vm.referenceLoadingForAccount(
                  index,
                  'primarySalesAttributionSubjectObjectId',
                ),
            }"
            :reference-options="{
              customerTypeId: vm.referenceOptionsForAccount(
                index,
                'customerTypeId',
              ),
              settlementMethodId: vm.referenceOptionsForAccount(
                index,
                'settlementMethodId',
              ),
              paymentMethodId: vm.referenceOptionsForAccount(
                index,
                'paymentMethodId',
              ),
              primarySalesAttributionSubjectObjectId:
            ...
            @search-reference="
              (key, keyword) => vm.searchReference(key, keyword, index)
            "
          />
          <template
            v-if="kind === 'editor' && account.accountId && vm.currentView"
          >
