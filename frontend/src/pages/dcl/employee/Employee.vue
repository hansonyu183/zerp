...
import { dclEmployeeStatusText } from './config'
import { dclEmployeeActiveVersion, type DclEmployeeListItem } from './types'
import { useDclEmployeeViewModel } from './vm'

const vm = reactive(useDclEmployeeViewModel())
const route = useRoute()
const router = useRouter()
const deleteTarget = ref<DclEmployeeListItem | null>(null)
const reviewTarget = ref<DclEmployeeListItem | null>(null)
const reviewComment = ref('')
const reverseTarget = ref<DclEmployeeListItem | null>(null)
...
function actions(row: DclEmployeeListItem): ListRowAction[] {
...
function selectAction(action: string, row: DclEmployeeListItem): void {
...
<template>
  <v-container fluid class="dcl-employee-page pa-5 pa-md-8">
      ...
      <template #cell-status="{ row }">
        <div class="dcl-status-chips">
          <v-chip density="comfortable" size="small" variant="tonal">{{
            dclEmployeeStatusText[dclEmployeeActiveVersion(row).approval.status]
          }}</v-chip>
          <v-chip
  ...
  <v-navigation-drawer
    ...
    class="dcl-employee-drawer"
    location="end"
    temporary
    width="760"
  >
    <div class="dcl-employee-drawer__content">
      <BusinessObjectEditor
        :editable="false"
        ...
        ><v-table class="responsive-table"
          ...
          <tbody>
            ...
            >
              ...
              <td>{{ dclEmployeeStatusText[item.approval.status] }}</td>
              <td>{{ item.data.employeeCategoryName ?? '—' }}</td>
              <td>{{ item.data.departmentName ?? '—' }}</td>
          ...
          <tbody>
            <tr v-for="event in vm.auditEvents" :key="event.id">
              ...
              <td>
                {{
                  event.fromStatus
                    ? dclEmployeeStatusText[event.fromStatus]
                    : '—'
                }}
                →
                {{
                  event.toStatus ? dclEmployeeStatusText[event.toStatus] : '—'
                }}
              </td>
              <td>{{ event.actorId }}</td>
