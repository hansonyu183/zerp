      ...
      <template #cell-status="{ row }">
        <div class="dcl-status-chips">
          <v-chip density="comfortable" size="small" variant="tonal">{{
            approvalStatusPresentation[
              dclEmployeeActiveVersion(row).approval.status
            ].label
          }}</v-chip>
          <v-chip
            v-if="row.latestApproved"
            :color="dclEmployeeActiveVersion(row).data.enabled ? 'success' : 'default'"
            density="comfortable"
            size="small"
            variant="tonal"
            >{{ dclEmployeeActiveVersion(row).data.enabled ? '启用' : '禁用' }}</v-chip
          >
          <v-chip
