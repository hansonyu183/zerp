...
export type DclEmployeeVersionView = components['schemas']['DclEmployeeVersionView']
...
export type DclEmployeeEditContext = { objectId: string; approvalEntryId: string; approvalRevision: number }
export type DclEmployeeConfig = {
  ...
  filters: readonly { key: 'status' | 'enabled'; label: string; type: 'select'; options: readonly BusinessObjectFieldOption[]; multiple?: boolean }[]
  emptyForm: () => DclEmployeeForm
}
export function dclEmployeeActiveVersion(item: Readonly<DclEmployeeListItem>): DclEmployeeVersionView {
