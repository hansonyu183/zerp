...
import { useDclFundAccountViewModel } from './vm'

const vm = reactive(useDclFundAccountViewModel())
const route = useRoute()
const router = useRouter()
const deleteTarget = ref<DclFundAccountListItem | null>(null)
...
function actions(row: DclFundAccountListItem): ListRowAction[] {
  ...
  if (available.enable || available.disable) {
    result.push({
      ...
      color: available.enable ? 'success' : 'warning',
    })
  }
...
function selectAction(action: string, row: DclFundAccountListItem): void {
...
<template>
  <v-container fluid class="dcl-fund-account-page pa-5 pa-md-8">
  ...
  <v-navigation-drawer
    ...
    class="dcl-fund-account-drawer"
    location="end"
    temporary
    width="720"
  >
    <div class="dcl-fund-account-drawer__content">
      <BusinessObjectEditor
      ...
      <v-alert
        ...
        type="warning"
        variant="tonal"
        >下方是正在变更的候选版本；新交易继续使用上方当前已批准版本，直到候选批准。</v-alert
      >
      <BusinessObjectEditor
        :editable="false"
...
<style scoped>
.dcl-fund-account-page {
  color: rgb(var(--v-theme-on-background));
}
.dcl-fund-account-drawer {
  background: rgb(var(--v-theme-background));
}
.dcl-fund-account-drawer__content {
  padding: 20px;
}
.dcl-status-chips {
  display: flex;
