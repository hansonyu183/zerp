...
<template>
  <v-container fluid class="pa-5 pa-md-8">
    ...
    >
      <template #filters>
        <v-select
          ...
          variant="outlined"
        />
        <v-checkbox v-model="vm.includeDisabled" label="包含停用" hide-details />
      </template>
    ...
    <v-dialog v-model="vm.editorOpen" max-width="980">
      <v-card>
        <v-card-title>{{ vm.selected ? '报表定义版本' : '新建报表定义' }}</v-card-title>
        <v-card-text>
          ...
          <div v-if="vm.selected" class="text-caption text-medium-emphasis">
            Approval Entry ID：{{ vm.selected.approval.approvalEntryId }} · 版本 {{ vm.selected.approval.versionNo }} · {{ approvalStatusPresentation[vm.selected.approval.status].label }}
          </div>
        </v-card-text>
        <v-card-actions class="flex-wrap">
          <v-btn @click="vm.editorOpen = false">关闭</v-btn>
          <v-btn
            v-if="!vm.selected || (vm.selected.approval.status === 'DRAFT' && vm.permissions.save)"
            color="primary"
            :loading="vm.saving"
            @click="vm.save"
          >保存</v-btn>
          <template v-if="vm.selected">
            <v-btn v-if="vm.selected.approval.status === 'DRAFT' && vm.permissions.submit" @click="vm.run('submit')">提交</v-btn>
            <v-btn v-if="vm.selected.approval.status === 'DRAFT' && vm.permissions['delete-version']" color="error" @click="vm.run('delete-version')">删除草稿</v-btn>
            <v-btn v-if="vm.selected.approval.status === 'PENDING' && vm.permissions.unsubmit" @click="vm.run('unsubmit')">撤回</v-btn>
            <v-btn v-if="vm.selected.approval.status === 'PENDING' && vm.permissions.reject" color="error" @click="vm.run('reject')">驳回</v-btn>
    ...
    <v-dialog v-model="vm.auditOpen" max-width="900">
      <v-card><v-card-title>审核记录</v-card-title><v-card-text>
        <v-table><thead><tr><th>时间</th><th>动作</th><th>Entry ID</th><th>原因</th></tr></thead>
          <tbody><tr v-for="event in vm.auditEvents" :key="event.id">
            <td>{{ event.createdAt }}</td><td>{{ approvalEventActionLabels[event.action] }}</td><td>{{ event.approvalEntryId }}</td><td>{{ event.reason ?? '—' }}</td>
          </tr></tbody></v-table>
      </v-card-text></v-card>
    </v-dialog>
  </v-container>
