...
import WarehouseDisableDialog from './WarehouseDisableDialog.vue'

const vm = reactive(useDclWarehouseViewModel())
const route = useRoute()
const router = useRouter()
const deleteTarget = ref<DclWarehouseListItem | null>(null)
...
function selectRowAction(action: string, row: DclWarehouseListItem): void {
...
<template>
  <v-container fluid class="dcl-warehouse-page pa-5 pa-md-8">
  ...
  <WarehouseDisableDialog :model="vm" />

  <v-navigation-drawer
    v-model="vm.drawerOpen"
    class="dcl-warehouse-drawer"
    location="end"
    temporary
    width="720"
  >
    <div class="dcl-warehouse-drawer__content">
      <BusinessObjectEditor
  ...
  <v-dialog
    ...
    "
  >
    <v-card rounded="xl" title="确认删除仓库变更草稿">
      <v-card-text>
...
.dcl-warehouse-drawer {
  background: rgb(var(--v-theme-background));
}

.dcl-warehouse-drawer__content {
  padding: 20px;
}
