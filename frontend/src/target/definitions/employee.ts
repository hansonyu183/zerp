  ...
  adapter: {
    ...
    get: async (token, id) => {
      ...
      return {
        ...
        options: {
          operatingEntityId: row.operatingEntity
            ? [summaryOption(row.operatingEntity)]
            : [],
          employeeCategoryId: row.employeeCategory
            ? [summaryOption(row.employeeCategory)]
            : [],
          departmentId: row.department ? [summaryOption(row.department)] : [],
          positionId: row.position ? [summaryOption(row.position)] : [],
        },
      }
    },
