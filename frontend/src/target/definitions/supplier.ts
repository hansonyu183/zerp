    ...
    validate: (snapshot) =>
      !snapshot.legalName.trim() || !snapshot.displayName.trim()
        ...
        : snapshot.defaultOperatingEntityId &&
          ...
          : null,
    query: (token, input) =>
      api.queryTargetSuppliers(token, {
        page: input.page,
        pageSize: input.pageSize,
        filters: input.keyword ? { keyword: input.keyword } : {},
      }),
    current: api.getTargetSupplier,
    submissions: (token, input) =>
      api.queryTargetSupplierSubmissions(token, {
        page: input.page,
        pageSize: input.pageSize,
        filters: input.keyword ? { keyword: input.keyword } : {},
      }),
    submission: api.getTargetSupplierSubmission,
    versions: api.queryTargetSupplierVersions,
    audit: api.queryTargetSupplierAuditHistory,
    submitNew: api.submitNewTargetSupplier,
    submitChange: api.submitChangeTargetSupplier,
    approve: api.approveTargetSupplier,
    reject: api.rejectTargetSupplier,
    unreject: api.unrejectTargetSupplier,
    unapprove: api.unapproveTargetSupplier,
    delete: api.deleteTargetSupplier,
    setEnabled: (token, item, enabled) =>
      api.setTargetSupplierEnabled(
        token,
