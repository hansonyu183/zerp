...
import { otherUnitListPage, useOtherUnitManagementViewModel } from './vm.ts'

const vm = reactive(useOtherUnitManagementViewModel())
onMounted(() => void vm.list.initialize())
onBeforeUnmount(vm.dispose)
const contractError = computed(() => {
  try {
    otherUnitListPage.validateRows(vm.list.items)
    return null
  } catch (cause) {
...
<template>
  <ManagementPageFrame :title="otherUnitListPage.title">
    <template #actions>
      <v-btn
        v-if="vm.canCreate()"
        color="primary"
    ...
    <template #alerts>
      ...
      <v-alert v-if="!vm.list.searchable" type="info"
        >当前账号没有其他单位正式资料读取权限。</v-alert
      >
    </template>
    <template #filters>
      <DynamicForm
        :fields="otherUnitListPage.filters"
    ...
    <DynamicCols
      :fields="otherUnitListPage.columns"
      :items="rows"
