...
import { supplierListPage, useSupplierManagementViewModel } from './vm.ts'

const vm = reactive(useSupplierManagementViewModel())
onMounted(() => void vm.list.initialize())
onBeforeUnmount(vm.dispose)
const contractError = computed(() => {
  try {
    supplierListPage.validateRows(vm.list.items)
    return null
  } catch (cause) {
...
<template>
  <ManagementPageFrame :title="supplierListPage.title">
    <template #actions>
      <v-btn
        v-if="vm.canCreate()"
        color="primary"
    ...
    <template #alerts>
      ...
      <v-alert v-if="!vm.list.searchable" type="info"
        >当前账号没有供应商正式资料读取权限。</v-alert
      >
    </template>
    <template #filters>
      <DynamicForm
        :fields="supplierListPage.filters"
    ...
    <DynamicCols
      :fields="supplierListPage.columns"
      :items="rows"
  ...
  >
    <v-card title="供应商提交">
      <v-card-text>
        ...
        />
        <v-select
          :model-value="vm.editor.draft.settlementMethod?.id ?? null"
          label="结算方式"
          clearable
          :items="
          ...
          "
          @update:model-value="vm.setSettlementMethod($event)"
        />
        <v-select
          :model-value="vm.editor.draft.defaultPurchaser?.objectId ?? null"
          label="默认采购员"
          clearable
          :items="
            vm.purchaserOptions.map((item) => ({
              title: `${item.code} · ${item.name}`,
              value: item.id,
            }))
          "
          @update:model-value="vm.setDefaultPurchaser($event)"
        />
