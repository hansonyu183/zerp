  ...
  <main>
    ...
    <template v-else>
      <section v-if="vouEntity" aria-label="目标单据">
        ...
        >
          <label v-for="entry in vouInputs(draft)" :key="entry.path.join('.')">
            ...
            <select
              v-if="
                entry.field.kind === 'enum' || entry.field.kind === 'boolean'
              "
            ...
            >
              <option
                v-for="value in entry.field.kind === 'boolean'
                  ? ['false', 'true']
                  : entry.field.enumValues"
                :key="value"
                :value="value"
              >
                {{ targetWireValueLabel(value) }}
              </option>
            </select>
            <template v-else>
              <select
                v-if="
                  ['objectId', 'assetId', 'billId'].includes(
              ...
              >
                ...
                <option
                  v-for="candidate in vouInputCandidates(entry)"
                  :key="`${candidate.entity}-${candidate.objectId}`"
          ...
          <label
            v-for="entry in vouArrayInputs(draft).filter(
              (item) => item.field.variants?.length,
            )"
          ...
          >
            ...
            <select @change="selectVouArrayVariant(draft, entry, $event)">
              <option
                ...
                :value="variant.id"
              >
                {{ targetWireValueLabel(variant.id) }}
              </option>
            </select>
          </label>
          <label
      ...
      <section v-else-if="accBookPage" aria-label="会计账簿">
        ...
        <article
          v-for="book in accBooks"
          :key="book.id"
          data-testid="acc-book"
          :data-acc-book-id="book.id"
        >
          <h3>{{ book.code }} · {{ book.name }}</h3>
          <label>名称<input v-model="book.name" /></label>
          <label>说明<textarea v-model="book.description" /></label>
          <label
            >本位币<input v-model="book.baseCurrency" maxlength="3"
          /></label>
          <p>{{ book.startMonth }}</p>
          <button
            type="button"
            :disabled="!canSaveAccBook"
            @click="saveAccBook(book)"
          >
            保存账簿
          </button>
          <button type="button" @click="deleteAccBook(book)">删除账簿</button>
          <button type="button" @click="selectAccBook(book.id)">
            选择账簿
          </button>
        </article>
      </section>
      <section v-else-if="accSubjectPage" aria-label="会计科目">
        <h2>会计科目</h2>
        <label
          >账簿<select v-model="accBookId" @change="selectAccBook(accBookId)">
            <option value="">请选择</option>
            <option v-for="book in accBooks" :key="book.id" :value="book.id">
              {{ book.code }} · {{ book.name }}
            </option>
          </select></label
        >
        <form aria-label="新建会计科目" @submit.prevent="createAccSubject">
          <label
            >新科目编码<input v-model="accSubjectCode" maxlength="64"
          /></label>
          <label
            >新科目名称<input v-model="accSubjectName" maxlength="200"
          /></label>
          <label
            >新科目方向<select v-model="accSubjectDirection">
              <option value="DEBIT">借方</option>
              <option value="CREDIT">贷方</option>
            </select></label
          >
          <button type="submit" :disabled="!canCreateAccSubject">
            新建科目
          </button>
        </form>
        <article
        ...
        >
          ...
          <button
            type="button"
            :disabled="!canSaveAccSubject"
            @click="saveAccSubject(subject)"
          >
            保存科目
          </button>
          <button type="button" @click="deleteAccSubject(subject)">
            删除科目
          </button>
        </article>
      </section>
      <section v-else-if="accOpeningPage" aria-label="账簿期初">
        ...
        <button
          type="button"
          :disabled="!canCreateOpeningDraft"
          @click="newOpeningDraft"
        >
          新建本地期初草稿
        </button>
        <article
          v-for="draft in openingDrafts"
          :key="draft.draftId"
          ...
          >
            ...
            <label
              >金额<input
                v-model="line.amount"
                inputmode="decimal"
                @input="saveOpeningDraft(draft)"
            /></label>
            <label
              >数量<input
                inputmode="decimal"
                :value="openingQuantity(line)"
                @input="updateOpeningQuantity(draft, line, $event)"
            /></label>
            <label
              >辅助维度（JSON）<textarea
                :value="JSON.stringify(line.dimensions)"
                @change="updateOpeningDimensions(draft, line, $event)"
              />
            </label>
            <button type="button" @click="deleteOpeningLine(draft, index)">
              删除明细
            </button>
          </fieldset>
          <button type="button" @click="addOpeningLine(draft)">
            新增期初明细
          </button>
          <label
            >资产登记（JSON 数组）<textarea
              data-testid="opening-assets"
              :value="openingCollectionJson(draft, 'assets')"
              @change="updateOpeningCollection(draft, 'assets', $event)"
            />
          </label>
          <label
            >票据登记（JSON 数组）<textarea
              data-testid="opening-bills"
              :value="openingCollectionJson(draft, 'bills')"
              @change="updateOpeningCollection(draft, 'bills', $event)"
            />
          </label>
          <label
            >空桶登记（JSON 数组）<textarea
              data-testid="opening-containers"
              :value="openingCollectionJson(draft, 'containers')"
              @change="updateOpeningCollection(draft, 'containers', $event)"
            />
          </label>
          <button type="button" @click="saveOpeningDraft(draft)">
            保存到本机
          </button>
          <button type="button" @click="submitOpeningDraft(draft)">提交</button>
          <button type="button" @click="deleteOpeningDraft(draft)">
            删除本地草稿
          </button>
        </article>
        <article v-if="accOpening" data-testid="acc-opening-submission">
          <h3>服务器期初提交件</h3>
          <p>
            {{ approvalStatusPresentation[accOpening.approval.status].label }}
          </p>
          <label>审批原因<input v-model="accReason" maxlength="1000" /></label>
          <button
            v-for="action in accOpening.availableApprovalActions"
            :key="action"
            type="button"
            :disabled="!canReviewAccOpening(action)"
            @click="reviewAccOpening(action)"
          >
            {{ approvalActionPresentation[action].label }}
          </button>
      ...
      <section v-else-if="accPeriodPage" aria-label="会计期间">
        ...
        <article
          v-for="period in accPeriods"
          :key="period.month"
          data-testid="acc-period"
        >
          <h3>{{ period.month }}</h3>
          <p>{{ period.locked ? '已锁定' : '未锁定' }}</p>
          <button
            v-if="period.locked"
            type="button"
            @click="setAccPeriod(false, period)"
          >
            解锁
          </button>
          <button v-else type="button" @click="setAccPeriod(true, period)">
            锁定
          </button>
        </article>
      </section>
      <section v-else-if="wflDefinitionPage" aria-label="流程定义维护">
        ...
        <button
          type="button"
          :disabled="!canCreateWflDefinitionDraft"
          @click="newWflDefinitionDraft"
        >
          新建本地流程草稿
        </button>
        <article
          v-for="draft in wflDrafts"
          :key="draft.draftId"
        ...
        >
          <label
            >脚本<textarea
              ...
              @input="saveWflDefinitionDraft(draft)"
            />
          </label>
          <label
            >试运行单据实体<input
          ...
          <label
            >试运行单据标识<input
              ...
              @input="saveWflDefinitionDraft(draft)"
          /></label>
          <button type="button" @click="saveWflDefinitionDraft(draft)">
            保存到本机
          </button>
          <button type="button" @click="trialWflDefinition(draft)">
            试运行
          </button>
          <p>
            试运行：{{
              draft.trialSucceeded ? '已通过' : '输入变更后需要重新试运行'
            }}
          </p>
          <button
            type="button"
            :disabled="!draft.trialSucceeded || !canSubmitWflDefinitionDraft"
            @click="submitWflDefinitionDraft(draft)"
          >
            提交
          </button>
          <button type="button" @click="deleteWflDefinitionDraft(draft)">
            删除本地草稿
          </button>
        </article>
        <article
          v-for="definition in wflDefinitions"
          :key="definition.submissionId"
          data-testid="wfl-definition-submission"
          :data-wfl-submission-id="definition.submissionId"
        >
          <h3>{{ definition.code }} · V{{ definition.versionNo }}</h3>
          <p>
            {{ approvalStatusPresentation[definition.status].label }} ·
            {{ definition.compiledGraph.name }}
          </p>
          <label
            >审批原因<input
              v-model="wflReasons[definition.submissionId]"
              maxlength="1000"
          /></label>
          <button
            v-for="action in definition.availableApprovalActions"
            :key="action"
            type="button"
            :disabled="!canReviewWflDefinition(definition, action)"
            @click="reviewWflDefinition(definition, action)"
          >
            {{ approvalActionPresentation[action].label }}
          </button>
          <button
            v-if="definition.status === 'APPROVED'"
            type="button"
            @click="setWflDefinitionEnabled(definition, !definition.enabled)"
          >
      ...
      <section v-else-if="wflCurrentPage" aria-label="当前流程定义">
        <h2>当前流程定义</h2>
        <article
          v-for="definition in wflCurrentDefinitions"
          :key="definition.approvalEntryId"
          data-testid="wfl-current-definition"
        >
          <h3>{{ definition.code }} · {{ definition.name }}</h3>
          <button
            type="button"
            @click="selectWflCurrentDefinition(definition.code)"
          >
            查看详情
          </button>
          <a :href="`/dcl/wfl-process-definition?code=${definition.code}`"
            >前往维护</a
          >
        </article>
        <pre v-if="wflCurrentDefinition">{{
          wflCurrentDefinition.compiledGraph
        }}</pre>
      </section>
      <section v-else-if="wflInstancePage" aria-label="流程实例">
        <h2>流程实例</h2>
        <article
          v-for="instance in wflInstances"
          :key="instance.processId"
          data-testid="wfl-instance"
        >
          <h3>{{ instance.definitionCode }} · {{ instance.definitionName }}</h3>
          <button type="button" @click="selectWflInstance(instance.processId)">
            查看实例
          </button>
        </article>
        <article v-if="wflInstance" data-testid="wfl-instance-detail">
          <h3>
            {{ wflInstance.definitionCode }} · {{ wflInstance.definitionName }}
          </h3>
          <fieldset v-for="node in wflInstance.nodes" :key="node.nodeId">
            <legend>{{ node.nodeName }}</legend>
            <p v-if="node.status">
              {{ approvalStatusPresentation[node.status].label }}
            </p>
            <label
              >原因<input v-model="wflReasons[node.nodeId]" maxlength="1000"
            /></label>
            <label
              >请求标识<input
                v-model="wflRequestKeys[node.nodeId]"
                minlength="16"
                maxlength="64"
            /></label>
            <button
              v-for="action in node.availableActions.filter(
                (value) => value !== 'CREATE_CHILD',
              )"
              :key="action"
              type="button"
              :disabled="!canActionWflInstance(node, action)"
              @click="actionWflInstance(node, action)"
            >
              {{ targetWireValueLabel(action) }}
            </button>
            <button
              v-for="target in wflInstance.availableTargets.filter(
                (item) =>
                  item.parentNodeId === node.nodeId &&
                  canActionWflInstance(node, 'CREATE_CHILD'),
              )"
              :key="target.targetNodeKey"
              type="button"
              @click="
                actionWflInstance(node, 'CREATE_CHILD', target.targetNodeKey)
              "
            >
              创建 {{ target.targetNodeName }}
            </button>
          </fieldset>
        </article>
      </section>
