...
test(
  ...
  async ({ page }) => {
    ...
    await page
      ...
      .fill(username)
    await page.getByLabel('显示名称', { exact: true }).fill('E2E 生命周期用户')
    await page.getByLabel('初始密码', { exact: true }).fill(initialPassword)
    ...
    await page
      .getByLabel(`操作 ${username}`, { exact: true })
    ...
    const temporaryPassword = await page
      ...
      .inputValue()
    await page.getByLabel('我已安全保存临时密码', { exact: true }).check()
