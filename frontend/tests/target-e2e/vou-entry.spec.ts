  ...
  for (const entity of entities) {
    for (const theme of [
      ...
      test(`${entity} menu candidates, input, persistence, readback and clone at ${width}px ${theme}`, async ({
        ...
        await expect(page.getByLabel('用户编码', { exact: true })).toHaveCount(
          0,
        )
        ...
        const drawer = page.locator('.v-navigation-drawer'),
          group = drawer
            ...
            .filter({ hasText: '业务单据' })
        if (
          !(await group.getAttribute('class'))?.includes('v-list-group--open')
        )
        ...
        if (
          ...
          if (entity.startsWith('employee-') || expenses)
            ...
            await editor
              ...
              .press('ArrowDown')
            await page
              .getByRole('option', { name: '员工', exact: true })
              .click()
        ...
        await expect(
          editor
            ...
            .locator('.mdi-send-outline'),
        ).toHaveAttribute('aria-hidden', 'true')
        if (
          ['asset-acquisition', 'bill-receipt', 'sales-receipt'].includes(
            entity,
          )
        ) {
          await expect(
          ...
          await page.screenshot({
            path: resolve(directory, `${entity}-${width}-${theme}.png`),
            animations: 'disabled',
          })
        }
        ...
        const get = page.waitForResponse(
          (response) =>
            new URL(response.url()).pathname === `/vou/${entity}/get`,
        )
        await page
