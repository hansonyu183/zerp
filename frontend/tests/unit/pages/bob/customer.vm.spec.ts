...
describe('customer relationship view model', () => {
  ...
  it('keeps query pagination fixed at 20 and rejects stale account results', async () => {
    ...
    mockedApiClient.postContract
      ...
      .mockResolvedValueOnce(listPage('new'))
  ...
  it('creates an account relationship with an existing Party', async () => {
    ...
    expect(mockedApiClient.postContract).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        partyId: 'party-1',
        data: expect.objectContaining({ operatingEntityId: 'ope-1' }),
      }),
    )
  })
  ...
  it('adds another account below the selected relationship', async () => {
    ...
    vm.detail.value = {
      ...
      operatingEntityCode: 'OPE-1',
      operatingEntityName: '华东',
      accounts: [],
      attachments: [],
    }
    vm.openAddAccount()
    vm.form.value.account.name = '第二结算户'
    vm.form.value.account.operatingEntity = {
      objectId: 'ope-1',
      approvalEntryId: '',
      code: 'OPE-1',
      name: '华东',
    ...
    vm.form.value.account.primarySalesAttribution.subject = {
      objectId: 'employee-1',
      approvalEntryId: '',
      code: 'EMP-1',
      name: '张三',
    ...
    expect(mockedApiClient.postContract).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ customerRelationshipId: 'relationship-1' }),
    )
  })
})
