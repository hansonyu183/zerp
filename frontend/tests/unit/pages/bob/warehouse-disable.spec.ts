...
describe('warehouse disable conflict labels', () => {
  ...
  it('maps blocking statuses and never exposes unknown wire values', () => {
    expect(warehouseDocumentStatusLabel('DRAFT')).toBe('草稿')
    expect(warehouseDocumentStatusLabel('CHECKED')).toBe('已核对')
    expect(warehouseDocumentStatusLabel('FUTURE_STATUS')).toBe('未知状态')
    expect(warehouseDocumentEntityLabel('future-document')).toBe('未知单据')
  })
})
