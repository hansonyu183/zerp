...
describe('DCL typed business archive strong identifiers', () => {
  ...
  it.each(['other-unit', 'sales-partner'] as const)(
    'round-trips every strong identifier when editing and disabling %s',
    async (entity) => {
      const view = {
        objectId: `${entity}-1`,
        entity,
      ...
      expect(vm.editorModel.value.strongIdentifiers).toEqual(
        view.data.strongIdentifiers,
      )
      ...
      expect(post).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          data: expect.objectContaining({
            strongIdentifiers: view.data.strongIdentifiers,
          }),
        }),
      )
      post.mockClear()
