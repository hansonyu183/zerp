...
function configureApi(): void {
  vi.mocked(targetApi.queryTargetWarehouses).mockResolvedValue(
    page([identity('warehouse')]) as never,
  )
  vi.mocked(targetApi.queryTargetFundAccounts).mockResolvedValue(
    page([identity('fund-account')]) as never,
  )
  vi.mocked(targetApi.queryTargetVehicles).mockResolvedValue(
    page([identity('vehicle')]) as never,
  )
...
describe('registered ListPage consumers', () => {
  ...
  it('mounts every registry definition through Host, queries once, displays contract fields, and cancels its dynamic editor without a refresh', async () => {
    const cases = [
      {
        domain: 'aux',
        entity: 'warehouse',
        query: vi.mocked(targetApi.queryTargetWarehouses),
        paths: ['/aux/warehouse/query', '/aux/warehouse/create'],
        editor: '新增仓库',
        code: 'warehouse-code',
        name: 'warehouse 名称',
      },
      {
        domain: 'aux',
        entity: 'fund-account',
        query: vi.mocked(targetApi.queryTargetFundAccounts),
        paths: ['/aux/fund-account/query', '/aux/fund-account/create'],
        editor: '新增资金账户',
        code: 'fund-account-code',
        name: 'fund-account 名称',
      },
      {
        domain: 'aux',
        entity: 'vehicle',
        query: vi.mocked(targetApi.queryTargetVehicles),
        paths: ['/aux/vehicle/query', '/aux/vehicle/create'],
        editor: '新增车辆',
        code: 'vehicle-code',
        name: 'vehicle 名称',
      },
