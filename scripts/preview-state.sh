...
copy_attachments(){ src=$1; dst=$2; mkdir -p "$dst"; chmod 700 "$dst"; [ -d "$src" ] && [ "$src" != "$dst" ] && cp -Rp "$src/." "$dst/" || true; }
...
claim(){ pr=${PREVIEW_PR:?PREVIEW_PR is required}; head=${PREVIEW_REF:?PREVIEW_REF is required}; actor=${PREVIEW_ACTOR:-${GITHUB_ACTOR:-unknown}}; if ! safe_number "$pr" || ! safe_sha "$head"; then echo "invalid PR or head" >&2; return 2; fi; [ "${PREVIEW_VERIFIED:-}" = 1 ] || { echo "exact validated PR head is required" >&2; return 1; }; init_state; reap; active_pr=$(cat "$active_file" 2>/dev/null || true); if [ -n "$active_pr" ] && [ "$active_pr" != "$pr" ]; then echo "preview slot is active for PR #${active_pr}" >&2; return 1; fi; if ! acquire_lock "$pr" "$head" "$actor"; then return 1; fi; base_sha=$(baseline_sha); base_record="$baseline_root/${base_sha}.state"; base_db=$(read_field db "$base_record"); base_att=$(read_field attachments "$base_record"); pr_record="$pr_root/${pr}.state"; pr_db="zerp_preview_pr_${pr}"; pr_att="$pr_root/${pr}/attachments"; if [ ! -f "$pr_record" ]; then clone_db "$base_db" "$pr_db"; copy_attachments "$base_att" "$pr_att"; generation=1; else generation=$(( $(read_field generation "$pr_record") + 1 )); fi; write_record "$pr_record" kind pr id "$pr" pr "$pr" sha "$head" db "$pr_db" attachments "$pr_att" generation "$generation" status active baseline "$base_sha" last_activity "$now" actor "$actor"; printf '%s\n' "$pr" >"$active_file"; chmod 600 "$active_file"; printf '%s\n' "$now" >"$lock_root/last_activity"; atomic_current pr "$pr" "$head" "$pr_db" "$pr_att" "$generation"; }
...
restore_baseline(){ pr=$(cat "$active_file" 2>/dev/null || true); if [ -n "$pr" ] && [ -f "$pr_root/${pr}.state" ]; then sha=$(read_field baseline "$pr_root/${pr}.state"); else sha=$(baseline_sha); fi; rec="$baseline_root/${sha}.state"; atomic_current baseline "$sha" "$sha" "$(read_field db "$rec")" "$(read_field attachments "$rec")" "$(read_field generation "$rec")"; }
...
resource_referenced(){ field=$1; value=$2; excluded=$3; for record in "$baseline_root"/*.state "$pr_root"/*.state; do [ -e "$record" ] || continue; [ "$record" = "$excluded" ] && continue; [ "$(read_field "$field" "$record")" = "$value" ] && return 0; done; return 1; }
