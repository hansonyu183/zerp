...
EOF
  ...
  if ! verify_active_release "$head" || \
    ...
    [ "$(read_field generation "$rec")" != "$generation" ]; then
    jq -n --arg url "https://zerp-preview.bytesucceed.com" --arg desc "preview changed after accepting PR #$pr head $head" '{state:"failure",environment_url:$url,description:$desc}' | $gh_bin api --method POST "repos/${repo}/deployments/${deployment}/statuses" --input - >/dev/null 2>&1 || true
    jq -n --arg state error --arg context full-validation --arg description "preview changed after acceptance for PR #$pr" --arg target_url "https://zerp-preview.bytesucceed.com" '{state:$state,context:$context,description:$description,target_url:$target_url}' | $gh_bin api --method POST "repos/${repo}/statuses/${head}" --input - >/dev/null 2>&1 || true
    echo "Preview changed after acceptance evidence was published" >&2
    return 1
  fi
...
promote(){ pr=${PREVIEW_PR:?PREVIEW_PR is required}; merge=${PREVIEW_MERGE_SHA:?PREVIEW_MERGE_SHA is required}; safe_sha "$merge" || return 2; [ "$(cat "$active_file" 2>/dev/null || true)" = "$pr" ] || { echo "PR is not active" >&2; return 1; }; src="$pr_root/${pr}.state"; [ "$(read_field status "$src")" = accepted ] || { echo "preview acceptance is required before promotion" >&2; return 1; }; head=$(read_field sha "$src"); GITHUB_REPOSITORY="$repo" GITHUB_SHA="$merge" ZERP_EXPECTED_PR="$pr" ZERP_EXPECTED_HEAD="$head" "${repo_root}/scripts/verify-merged-pr.sh" >/dev/null; db=$(read_field db "$src"); att=$(read_field attachments "$src"); write_record "$baseline_root/${merge}.state" kind baseline id "$merge" sha "$merge" db "$db" attachments "$att" generation "$(read_field generation "$src")" status accepted accepted_at "$now"; atomic_current baseline "$merge" "$merge" "$db" "$att" "$(read_field generation "$src")"; sed "s/^status=.*/status=promoted/" "$src" >"${src}.new"; mv -f "${src}.new" "$src"; rm -f "$active_file"; release_lock; }
