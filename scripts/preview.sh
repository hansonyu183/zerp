...
SELECT 'CREATE ROLE zerp_report_reader NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS'
WHERE NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='zerp_report_reader') \gexec
...
GRANT zerp_report_reader TO ${db_migration_user};
GRANT zerp_report_reader TO ${POSTGRES_USER};
EOF
